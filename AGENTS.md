...
- 변경 이력: 발행(Released) 장은 각주 아래 “## 변경 이력” 섹션을 두고, 변경 사항을 날짜·요약으로 누적 기록합니다(초안은 선택).
...
- 사소한 수정(철자/표현/링크/형식/소규모 예시 보정): 상태를 유지하고, 6) 변경 이력 섹션에 날짜·요약을 추가합니다.
- 의미 있는 개정(학습 목표 변화, 구조 재편, 섹션 대폭 추가/삭제, 예시/템플릿 전면 수정): 메이저 버전을 +1 하고 상태를 `v{n+1}-draft`로 전환합니다. `book/index.md`와 `README.md`를 동기화하고, 변경 이력에 전환 사유를 남깁니다.
- 발행 후 변경 누적: `v1-released` 상태에서도 변경 이력은 계속 누적합니다(상태는 그대로 유지). 충분한 변경이 축적되어 의미 있는 개정이면 `v2-draft`로 승격합니다.
