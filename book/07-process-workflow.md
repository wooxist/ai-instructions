...
## 다음 단계: 더 많은 에이전트가 필요할 때

7장에서는 1-2개 에이전트로 구성된 기본 워크플로우를 배웠습니다. 하지만 실제 업무에서는 3-4개 이상의 에이전트가 팀처럼 협력해야 하는 경우가 많습니다.

**다음 신호가 보이면 7.5장으로 넘어가세요**:
- ✅ 3개 이상의 명확히 구분되는 역할이 필요할 때
- ✅ 파일이 많아져서 디렉토리 구조가 필요할 때
- ✅ "지금 어느 단계까지 진행됐는가?"를 추적하고 싶을 때
- ✅ 실패한 단계만 재실행하고 싶을 때

[7.5장 중간 규모 협업과 파일 기반 관리](07-5-intermediate-collaboration.md)에서는 다음을 배웁니다:
- 3-4개 에이전트로 구성된 팀 협업 패턴
- 작업별 디렉토리 구조 설계 (`/tasks`, `/outputs`, `/shared`)
- `status.json`을 사용한 상태 추적
- JSON 기반 산출물 표준화

**참고**: 7.5장은 선택사항입니다. 1-2개 에이전트로 충분하다면 바로 8장(성능 최적화)으로 넘어가도 됩니다. 더 복잡한 조직 구조가 필요한 경우에만 10장으로 가세요.
