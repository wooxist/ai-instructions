...
## 다음 단계 선택 가이드: 당신의 프로젝트 규모는?

7장을 완료하셨습니다! 이제 당신의 프로젝트 규모에 맞는 다음 단계를 선택할 시간입니다.

### 🎯 상황 1: 1-2개 에이전트로 충분한 경우

**이런 신호가 보이나요?**
- ✅ 워크플로우가 단순하고 명확함 (3-4단계 이내)
- ✅ 파일 구조가 평면적 (모든 파일이 같은 폴더)
- ✅ 상태 추적이 필요 없음 (실패 시 처음부터 재실행 가능)

**→ [8장. 성능 최적화](08-performance.md)로 이동하세요** (★★☆)

에이전트 수를 늘리기보다, 현재 에이전트의 응답 속도와 품질을 개선하는 것이 더 효과적입니다.

---

### 🎯 상황 2: 3-4개 에이전트가 필요한 경우 (중간 규모)

**이런 신호가 보이나요?**
- ✅ 3-4개의 명확히 구분되는 역할이 필요
- ✅ 파일이 많아져서 디렉토리 구조가 필요 (`/tasks`, `/outputs`)
- ✅ "지금 어느 단계까지 진행됐는가?" 추적 필요
- ✅ 실패한 단계만 재실행하고 싶음
- ✅ 각 에이전트의 산출물을 JSON으로 표준화하고 싶음

**→ [7.5장. 중간 규모 협업과 파일 기반 관리](07a-intermediate-collaboration.md)로 이동하세요** (★★★)

**⚠️ 난이도 점프 경고**: 7장(★★☆)에서 7.5장(★★★)으로 넘어가면 난이도가 한 단계 올라갑니다:
- **새로운 개념**: 디렉토리 기반 상태 관리, JSON 기반 인터페이스
- **학습 시간**: 약 2-3시간
- **전제 조건**: 7장의 워크플로우 패턴을 확실히 이해해야 함

7.5장을 완료하면, 이후 10장(고급 협업 아키텍처)으로 자연스럽게 진행할 수 있습니다.

---

### 🎯 상황 3: 6개 이상 에이전트가 필요한 경우 (대규모)

**이런 신호가 보이나요?**
- ✅ 에이전트가 5-6개 이상 필요
- ✅ 에이전트가 다른 에이전트를 동적으로 생성해야 함
- ✅ "누가 어떤 에이전트를 만들 것인가?"가 동적으로 결정됨
- ✅ 여러 프로젝트가 동시에 진행됨
- ✅ 메타/아키텍트/워커 계층 구조가 필요

**→ [7.5장](07a-intermediate-collaboration.md)을 먼저 학습 후, [10장. 고급 협업 아키텍처](10-advanced-collaboration-architectures.md)로 이동** (★★★)

**⚠️ 난이도 점프 경고**: 7장에서 바로 10장으로 가면 큰 난이도 격차가 있습니다:
- **난이도 차이**: ★★☆ → ★★★ (2단계 점프)
- **새로운 개념**: 계층적 에이전트 구조, 동적 생성, 다중 작업 관리
- **권장 학습 경로**: 7장 → 7.5장 → 10장 (점진적 학습)
- **직행 가능 조건**: 이미 복잡한 시스템 설계 경험이 있는 경우에만

**중요**: 대부분의 실무 프로젝트는 3-4개 에이전트(7.5장 수준)로 충분합니다. 10장의 복잡한 구조는 실제로 필요할 때만 도입하세요.
...
1. **보수적으로 시작**: 8장으로 가서 성능을 최적화한 후, 필요하면 돌아와서 7.5장을 학습하세요.
2. **실험적 접근**: 7.5장을 한 번 훑어보고, 필요 없다고 판단되면 8장으로 이동하세요.
3. **시간 여유가 있다면**: 7.5장을 학습하면 향후 프로젝트 확장 시 큰 도움이 됩니다.

**다음으로 권장하는 학습 경로:**
- **대부분의 독자**: 8장 → 9장 → 11장 (실무 중심)
- **고급 독자**: 7.5장 → 10장 → 13장 (아키텍처 중심)
...
8장은 "일잘러의 성능 튜닝 비법"입니다. 워크플로우는 잘 만들었는데 느리거나 비싸다면, [8장으로 이동하세요 →](08-performance.md)
