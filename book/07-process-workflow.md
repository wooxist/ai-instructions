...
메타 에이전트[^5]는 이 책과 프로젝트가 지향하는 궁극적인 목표이자, 가장 진보된 형태의 에이전트 아키텍처입니다.

일반 에이전트가 '주어진 문제'를 해결하는 것과 달리, 메타 에이전트는 **"문제를 해결하기 위한 에이전트 팀과 워크플로우 자체를 설계하고 관리하는"** 에이전트입니다. 즉, 사용자가 "신규 고객 온보딩 프로세스를 자동화해줘"와 같은 고수준의 목표만 제시하면, 메타 에이전트가 스스로 필요한 에이전트들의 역할(`agents/*.md`)과 협력 방식(`workflow.yaml`)을 동적으로 생성하고 최적화하여 문제를 해결합니다.

이러한 메타 에이전트는 역할에 따라 다음과 같이 전문화될 수 있습니다.
- **아키텍트 에이전트:** 새로운 요구사항에 맞춰 시스템의 구성요소(신규 에이전트, 워크플로우)를 '설계하고 생성'합니다.
- **최적화 에이전트:** 기존 시스템의 성능 로그나 비용 데이터를 분석하여, 더 효율적으로 만들기 위해 기존 인스트럭션이나 워크플로우를 '수정하고 개선'합니다.
...
[^5]: **메타 에이전트(Meta Agent):** '메타(Meta)'는 '~에 대한', '~을 넘어서'라는 의미를 가진다. 따라서 메타 에이전트는 다른 에이전트나 에이전트 시스템 자체를 '대상으로' 생각하고 관리하는 한 단계 위의 에이전트를 의미한다.
