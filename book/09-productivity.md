...
## 🔮 다음 장 미리보기: 10장 - 실전 패턴: 단일 에이전트 설계

**"평가 방법을 배웠다면, 이제 실전 패턴을 익힐 차례입니다."**

9장에서 배운 평가와 검증 방법은 **인스트럭션의 품질**을 높이는 데 집중했습니다. 이제 Part 4에서는 지금까지 배운 모든 원칙과 기법을 **실제 업무 시나리오**에 적용하는 방법을 배웁니다.

### 10장에서 배울 핵심 내용

**🎯 3가지 복잡도 수준의 실전 사례**
- **간단/일상**: 이메일 요약 - 빠르게 반복되는 일상 작업
- **표준/전문**: 마케팅 문구 생성 - 전문성이 필요한 콘텐츠 작업
- **복잡/중요**: 회의록 할 일 추출 - 구조화된 출력이 필요한 중요 작업

**🏗️ 단일 에이전트의 핵심 패턴**
- **사용자가 메타 에이전트**: 목표 설정 → 에이전트 생성 → 실행 감독
- **역할/목표/제약의 균형**: 각 복잡도에 맞는 인스트럭션 설계
- **구조화된 출력**: JSON 스키마로 후속 처리 용이성 확보

**📝 템플릿 활용**
- 3가지 사례를 템플릿으로 활용하여 자신의 업무에 적용
- Part 1-3에서 배운 원칙들이 실제로 어떻게 구현되는지 확인
- 9장의 A/B 테스트로 템플릿을 개선하는 방법

### 9장과 10장의 연결점

| 9장 (평가와 검증) | 10장 (실전 패턴) |
|---|---|
| A/B 테스트 방법 학습 | 실제 사례로 테스트 적용 |
| 평가 지표 정의 | 지표로 패턴 효과 측정 |
| 오답 노트 운영 | 실전 실패 사례 축적 |

**학습 팁**: 10장은 **★★☆ (중급)** 난이도로, Part 3까지 학습한 내용을 실전에 적용하는 총정리 장입니다. 각 사례를 자신의 업무에 맞게 변형해보세요.

**준비되셨나요?** [10장: 실전 패턴: 단일 에이전트 설계](10-single-agent.md)에서 배운 것을 실전에 적용해봅시다! 🚀
...
- **7장 - 프로세스와 워크플로우**: 기본 패턴(Pipeline, Generate-and-Verify, Routing)과 워크플로우 설계
...
- [ ] **워크플로우 설계**: Pipeline, Generate-and-Verify 같은 기본 패턴을 이해하고 적용할 수 있다
...
이제 **Part 4**에서는 **실전 사례**를 통해 지금까지 배운 내용을 종합적으로 적용합니다:

- **10장 - 실전 패턴: 단일 에이전트 설계**: 3가지 복잡도 수준의 실무 사례
  - 간단/일상: 이메일 요약
  - 표준/전문: 마케팅 문구 생성
  - 복잡/중요: 회의록 할 일 추출

**Part 1-3에서 배운 원칙, 설계 방법, 평가 기법이 실제 업무 시나리오에서 어떻게 구현되는지 확인하세요.** 10장의 템플릿을 활용하여 자신만의 인스트럭션을 설계할 수 있습니다.

**이것이 1권의 마지막 장입니다!** 1권 완료 후, 2권에서는 다중 에이전트 협업과 조직 설계를 다룹니다.

**준비되셨다면, [10장: 실전 패턴: 단일 에이전트 설계](10-single-agent.md)로 시작하세요!**
