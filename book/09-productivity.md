...
**오답 노트 템플릿 (권장 필드/주기/회귀 방지 절차)**

```markdown
# 오답 노트: [작업명/시나리오]
날짜: YYYY-MM-DD
인스트럭션 버전: vX.Y (변경 이력 링크)
...
## 수정 사항
- 변경 내용: [프롬프트/제약/입출력 명세 변경]
- 영향 범위: [영향 받는 워크플로우/에이전트]

## 회귀 방지 체크리스트
- [ ] 재현 케이스를 회귀 테스트 세트에 추가
- [ ] 성공 기준에 해당 항목 추가(검증 자동화 가능하면 스크립트/에이전트 포함)
- [ ] 유사 작업(Top-N)에 대해 스팟 체크 실행
...
## 🔮 다음 장 미리보기: 10장 - 고급 협업 아키텍처

**"A/B 테스트로 개별 에이전트를 개선했다면, 이제 여러 에이전트가 협력하는 시스템을 설계할 차례입니다."**

9장에서 배운 평가와 검증 방법은 **개별 에이전트의 품질**을 높이는 데 집중했습니다. 하지만 실제 비즈니스 문제는 한 명의 전문가가 해결할 수 없는 경우가 대부분입니다. 신제품 출시 캠페인, 고객 온보딩, 콘텐츠 제작 파이프라인... 이런 복잡한 업무는 **여러 전문가의 협업**이 필요합니다.

### 10장에서 배울 핵심 내용

**🏗️ 계층적 조직 설계**
- **메타 에이전트**: 에이전트를 만드는 에이전트 (조직의 '인사부')
- **아키텍트 에이전트**: 프로젝트를 총괄하는 관리자 (조직의 '프로젝트 매니저')
- **워커 에이전트**: 실제 작업을 수행하는 전문가 (조직의 '실무자')

**📋 산출물 인터페이스**
- 에이전트 간 소통의 규칙: JSON/YAML로 명확한 '계약' 정의
- 산출물이 곧 로그: 작업 추적과 디버깅이 쉬워짐
- 9장의 '오답 노트'를 시스템 전체로 확장하는 방법

**🗂️ 작업(Job) 관리**
- `/jobs/` 디렉토리로 여러 프로젝트를 동시 관리
- `_job_log.json`으로 작업 상태 추적
- 실패한 작업의 재시도와 복구 전략
...
| 9장 (개별 개선) | 10장 (시스템 설계) |
|---|---|
| A/B 테스트로 프롬프트 비교 | 여러 에이전트의 협업 패턴 비교 |
| 오답 노트로 개인 학습 | 시스템 전체의 실패 추적 |
| 평가 지표로 품질 측정 | 작업 로그로 시스템 성능 측정 |

**난이도 점프 경고 ⚠️**: 10장은 **★★★ (고급)** 난이도입니다. 7.5장(중간 규모 협업)을 먼저 학습하면 10장이 훨씬 수월합니다.

**준비되셨나요?** [10장: 고급 협업 아키텍처](10-advanced-collaboration-architectures.md)에서 복잡한 AI 조직을 설계하는 방법을 배워봅시다! 🚀
...
## Part 3 완료: 워크플로우와 성능 최적화를 마스터했습니다

축하합니다! **Part 3: 워크플로우, 성능과 평가**의 모든 장을 학습하셨습니다. 이제 다음 내용을 마스터하셨습니다:

- **7장 - 프로세스와 워크플로우**: 기본 패턴(Creator-Evaluator, Coordinator)과 다중 에이전트 협업 설계
- **8장 - 성능 최적화**: 토큰 사용량 감소, 응답 속도 향상, 비용 절감 기법
- **9장 - 평가와 검증**: A/B 테스트, 평가 지표, 오답 노트로 지속적 개선

### 학습 성과 체크

다음 내용을 확인하고 다음 Part로 넘어가세요:

- [ ] **워크플로우 설계**: 단일 에이전트의 한계를 넘어, Creator-Evaluator 같은 협업 패턴을 설계할 수 있다
- [ ] **성능 최적화**: 토큰 밀도 증가, 요약, 캐싱으로 효율을 높일 수 있다
- [ ] **객관적 평가**: 재작업 비율, 만족도, 채택률 같은 지표로 품질을 측정하고 개선할 수 있다
- [ ] **지속적 개선**: A/B 테스트와 오답 노트로 회귀를 방지하며 반복적으로 개선할 수 있다

---

## 다음: Part 4 - 아키텍처와 설계 패턴

이제 **Part 4**에서는 **복잡한 다중 에이전트 시스템**을 설계하는 고급 기법을 배웁니다:

- **10장 - 고급 협업 아키텍처**: 6개 이상의 에이전트가 협력하는 복잡한 시스템 설계
- **11장 - 설계 패턴 사례집**: 단일 에이전트부터 복합 조직까지 5가지 실전 사례
  - 11.1부: 단일 에이전트 패턴
  - 11.2부: 단위 조직 패턴 (3-5개 에이전트)
  - 11.3부: 복합 조직 패턴 (6개 이상)

**Part 3에서 기본적인 워크플로우를 배웠다면, Part 4에서는 메타-아키텍트-워커의 3계층 구조와 병렬 처리가 필요한 엔터프라이즈급 시스템을 만드는 법을 배웁니다.** 이제 단순히 "여러 AI를 쓴다"가 아니라, 제대로 "설계된 조직"을 구축하는 단계로 나아갑니다.

**준비되셨다면, [10장: 고급 협업 아키텍처](10-advanced-collaboration-architectures.md)로 시작하세요!**
