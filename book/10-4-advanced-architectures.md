...
# 처리 방법
1. `[#기초 지식 및 원칙]`을 바탕으로, `[#입력 데이터]`를 분석하여 아키텍트 에이전트가 비용 최적화 능력을 갖추기 위해 필요한 새로운 '역할'과 '처리 방법'을 정의한다.
2. 기존 `architect_agent.md`의 내용을 분석한다.
3. 아키텍트 에이전트의 `#처리 방법`에 "시스템의 성능 로그를 분석하여 비용이 많이 발생하는 지점을 식별하고, 비용 절감 가설을 수립하여 인스트럭션을 수정하는" 단계를 추가한다.
4. 아키텍트 에이전트의 `#기초 지식 및 원칙`에 "성능의 삼각형"과 "데이터 기반 평가" 원칙을 명시적으로 추가하여, 의사결정의 기준을 제공한다.
5. 위 내용을 반영하여, 개선된 `architect_agent_v2.md` 인스트럭션을 생성한다.

# 출력물
- **개선된 아키텍트 에이전트 인스트럭션**: `architect_agent_v2.md`의 전체 내용.
```
