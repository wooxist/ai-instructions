...
- **스콥(Scope)의 명확화:** 이 아키텍트 에이전트의 핵심 역할은 비즈니스 과업을 직접 수행하는 것이 아니라, **시스템의 구조 자체(agent.md, workflow.yaml)를 생성하고 수정**하는 상위 레벨의 작업에 집중합니다. 이는 4장에서 배운 **관심사 분리(SoC)** 원칙을 메타 레벨에서 적용한 것입니다.
...
- **단일 진실 공급원 (SSOT):** 최적화 에이전트는 중앙에서 관리되는 인스트럭션 파일과 워크플로우 정의(`workflow.yaml`)와 같은 **단일 진실 공급원**을 수정 대상으로 삼아, 시스템 전체의 일관성을 유지하며 개선 작업을 수행합니다.
