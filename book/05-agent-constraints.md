...
- **Rule of Thumb (경험 법칙):** `[에이전트의 인스트럭션 총 길이] + [평균 입력 데이터의 길이] > [모델 컨텍스트 윈도우의 70%]` 라면, 에이전트 분리를 강력하게 고려해야 합니다. 컨텍스트 윈도우가 꽉 차면 AI의 성능이 급격히 저하되기 때문입니다.
- 즉, 하나의 에이전트가 너무 많은 역할과 지식을 가지게 되어 인스트럭션 자체가 너무 길어지면, 실제 데이터를 처리할 공간이 부족해지므로 반드시 분리해야 합니다.

### 5.3.2 2단계: 설계 원칙에 따른 분리 결정

컨텍스트 윈도우라는 물리적 제약을 만족했다면, 이제 더 나은 설계를 위해 4가지 소프트웨어 공학 원칙을 적용하여 분리 여부를 결정합니다.
...
그렇다면 에이전트의 인스트럭션은 실질적으로 몇 토큰 정도가 적절할까요? 정답은 없지만, **'토큰 예산'** 개념을 도입하면 효과적으로 판단할 수 있습니다. 모델의 전체 컨텍스트 윈도우를 '총 예산'으로 보고, 이 예산을 '인스트럭션', '입력 데이터', '출력 데이터' 세 항목에 배분하는 것입니다.

`총 예산 (모델 컨텍스트 윈도우) = 인스트럭션 토큰 + 입력 데이터 토큰 + 출력 데이터 토큰`

**경험 법칙 (Rules of Thumb):**

1.  **인스트럭션-데이터 비율:** 좋은 시작점은 에이전트의 자체 인스트럭션(역할, 처리 방법, 제약 등)이 **전체 컨텍스트 윈도우의 20~30%를 넘지 않도록** 설계하는 것입니다. 그래야 실제 작업에 필요한 입력 데이터와 AI가 생성할 출력물을 위한 충분한 공간(70~80%)을 확보할 수 있습니다.
...
### 5.4.2 [컨텍스트 윈도우 문제 해결] → '역할 분할'과 '명시적 제약' 설계

긴 지시를 잊어버리는 문제를 해결하기 위해, 복잡한 작업을 여러 에이전트의 **역할**로 나누어 컨텍스트 윈도우의 부담을 근본적으로 줄여야 합니다. 이는 **관심사 분리(SoC)** 원칙의 가장 중요한 적용 사례입니다. 또한, RAG(Retrieval-Augmented Generation)[^4] 기술을 활용해 필요한 정보만 동적으로 주입하는 것도 효과적입니다.
