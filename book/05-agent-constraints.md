...
### 5.3.1 1단계: 물리적 제약 확인 - 컨텍스트 창 크기

에이전트 설계의 가장 첫 번째이자 가장 중요한 기준은 **"인스트럭션과 데이터가 모델의 컨텍스트 창 안에 들어가는가?"** 입니다.

- **Rule of Thumb (경험 법칙):** `[에이전트의 인스트럭션 총 길이] + [평균 입력 데이터의 길이] > [모델 컨텍스트 창의 70%]` 라면, 에이전트 분리를 강력하게 고려해야 합니다. 컨텍스트 창이 꽉 차면 AI의 성능이 급격히 저하되기 때문입니다.
- 즉, 하나의 에이전트가 너무 많은 역할과 지식을 가지게 되어 인스트럭션 자체가 너무 길어지면, 실제 데이터를 처리할 공간이 부족해지므로 반드시 분리해야 합니다.

### 5.3.2 2단계: 설계 원칙에 따른 분리 결정

컨텍스트 창이라는 물리적 제약을 만족했다면, 이제 더 나은 설계를 위해 4가지 소프트웨어 공학 원칙을 적용하여 분리 여부를 결정합니다.

- **원칙 1: 전문성 (Expertise):** 책임들이 서로 다른 전문 지식을 요구하는가?
  - 예: 'UI 디자인'과 '데이터베이스 스키마 설계'는 완전히 다른 전문성이므로, `UI 에이전트`와 `DB 에이전트`로 분리해야 합니다.

- **원칙 2: 변경 이유 (Reason to Change) - 단일 책임 원칙[^6]**
  - 책임들의 변경 이유가 다른가?
  - 예: 'API 명세 변경'과 'UI 디자인 트렌드 변경'은 서로 다른 이유로 발생하므로, `API 연동 에이전트`와 `UI 위젯 에이전트`로 분리하는 것이 좋습니다.

- **원칙 3: 재사용성 (Reusability):** 책임의 일부가 다른 워크플로우에서도 재사용될 수 있는가?
  - 예: '코드 리뷰' 책임은 어떤 코드 생성 작업에서든 재사용될 수 있으므로, 독립적인 `코드 리뷰 에이전트`로 만드는 것이 효율적입니다.

- **원칙 4: 응집도 (Cohesion)[^7]:** 책임들이 '하나의 목적'을 위해 긴밀하게 묶여 있는가?
  - 예: '버튼, 텍스트 필드, 드롭다운 메뉴 생성'은 모두 'UI 위젯 제작'이라는 단일 목적을 가지므로, `공통 위젯 에이전트` 하나로 묶는 것이 응집도가 높습니다.

### 5.3.3 에이전트 인스트럭션의 적정 토큰 크기

그렇다면 에이전트의 인스트럭션은 실질적으로 몇 토큰 정도가 적절할까요? 정답은 없지만, **'토큰 예산'** 개념을 도입하면 효과적으로 판단할 수 있습니다. 모델의 전체 컨텍스트 창을 '총 예산'으로 보고, 이 예산을 '인스트럭션', '입력 데이터', '출력 데이터' 세 항목에 배분하는 것입니다.

`총 예산 (모델 컨텍스트 창) = 인스트럭션 토큰 + 입력 데이터 토큰 + 출력 데이터 토큰`

**경험 법칙 (Rules of Thumb):**

1.  **인스트럭션-데이터 비율:** 좋은 시작점은 에이전트의 자체 인스트럭션(역할, 처리 방법, 제약 등)이 **전체 컨텍스트 창의 20~30%를 넘지 않도록** 설계하는 것입니다. 그래야 실제 작업에 필요한 입력 데이터와 AI가 생성할 출력물을 위한 충분한 공간(70~80%)을 확보할 수 있습니다.
    - 예: 128k 컨텍스트 창을 가진 모델이라면, 에이전트 인스트럭션 자체는 약 25k~38k 토큰 미만으로 유지하는 것이 이상적입니다.

2.  **최소 기능 인스트럭션 (Minimum Viable Instruction):** 처음부터 완벽하고 긴 인스트럭션을 만들기보다, 작업을 수행할 수 있는 가장 짧고 간단한 버전으로 시작하세요. 그리고 결과물의 품질이 만족스럽지 않을 때만, 페르소나, 예시, 제약 조건 등을 추가하며 점진적으로 길이를 늘려나가는 것이 좋습니다. (**점진적 개선** 원칙)
...
4.  **비용을 제약 조건으로 활용:** 비용이 중요한 요소라면, 이는 에이전트 설계의 강력한 제약 조건이 됩니다. 비싼 모델을 사용하면서 긴 인스트럭션을 유지하기 어렵다면, 자연스럽게 하나의 복잡한 에이전트를 두 개의 더 작고 저렴한 에이전트로 분리하게 될 것입니다.
...
### 5.4.1 [할루시네이션 문제 해결] → 근거 기반 '책임'과 '검증 에이전트' 설계
...
### 5.4.2 [컨텍스트 창 문제 해결] → '역할 분할'과 '명시적 제약' 설계
...
### 5.4.3 [비일관성 문제 해결] → 명확한 '페르소나'와 '구조화된 출력' 설계
...
### 5.4.4 [유지보수/확장성 문제 해결] → '모듈화'된 에이전트 설계
...
이 장에서 배운 개념들을 종합하여 실제 파일 기반 인스트럭션 시스템으로 구축하는 전체 과정은 [TEMP_10장. 실무 활용하기](10-practical.md)에서 자세히 다룹니다.
