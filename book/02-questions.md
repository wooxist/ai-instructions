...
### 언제 유용한가
승인/거부, 충족/미충족, 규정 준수 점검, 릴리즈 고/노고 등

### 언제 안 쓰나
문제가 모호하거나 기준이 미정인 초기 단계, 설명/아이디어가 필요한 상황.

### 예시
"이 PR은 보안 가이드라인 4항목(입력검증, 비밀번호 해시, 로그 마스킹, 비밀키 노출 금지)을 충족합니까? 예/아니오 + 근거 1줄"
...
### 이렇게 적어보세요
"다음 기준을 모두 충족합니까? 예/아니오와 근거 1줄. 기준: ① … ② … ③ …"

### 자주하는 실수
‘충분히 빠름’ 같은 모호한 기준, 정의되지 않은 용어

### 더 좋게
기준을 체크리스트로 수량화하고, '예/아니오' 뒤에 한 줄 근거를 요구하세요.

## 2.2 개방형 질문 (Open-ended)
설명·요약·서술이 필요한 장면에서 맥락을 풍부하게 해 줍니다.

### 언제 유용한가
배경 설명, 요약/정리, 개념 설명, 사례 수집

### 언제 안 쓰나
즉시 결정을 내려야 하거나 기준 충족 여부만 확인하면 되는 상황(폐쇄형/비교형 적합).

### 예시
"임원 보고용으로 프로젝트 A 경과를 5줄 불릿으로 요약해줘. 톤은 간결/결정지향. 마지막 줄에 다음 액션 1개"
...
### 자주하는 실수
범위를 넓게 두면 잡음이 늘어납니다. 대상·기간·자료원을 함께 적으세요.

### 더 좋게
형식(표/불릿/마크다운)과 길이를 지정하면 결과의 들쭉날쭉함이 줄어듭니다.

## 2.3 탐색형 질문 (Exploratory)
아이디어를 넓게 펼치고 대안을 모을 때 좋습니다. 개수·다양성·평가기준을 함께 제시하면 품질이 올라갑니다.

### 언제 유용한가
브레인스토밍(brainstorming)[^4], 기능 후보 발굴, 리스크 식별

### 언제 안 쓰나
환경·제약이 명확하고 실행 계획/검증이 필요한 단계(맥락 의존형/폐쇄형/비교형 적합).

### 예시
"초기 MVP[^2] 기능 후보 5개를 제안하고, 구현 난이도/가치/리스크 평가와 간단 근거를 표로 정리"
...
### 이렇게 적어보세요
"N가지 대안을 제안하고, 각 대안의 장단점/적용 전제/10점 만점 자체 평점을 적어줘"

### 자주하는 실수
현실 제약이 없으면 실행 불가 아이디어가 늘어납니다. 제약을 꼭 함께 명시하세요.

### 더 좋게
‘중복 금지’·‘카테고리 다양성’ 조건을 넣어 편향을 줄이세요.

## 2.4 비교형 질문 (Comparative)
명확한 기준으로 옵션을 나란히 놓고 선택할 때 씁니다.

### 언제 유용한가
도구/프레임워크 선택, 아키텍처 대안 비교, 우선순위 결정

### 언제 안 쓰나
비교 대상·기준이 불명확하거나 하나의 옵션만 있는 경우(탐색형/개방형으로 전단계 정리).

### 예시
"LangChain과 LlamaIndex를 비용/문서화/커뮤니티/확장성 기준으로 비교 표 작성 + 추천과 사유"
...
### 이렇게 적어보세요
"비용/성능/학습곡선(learning curve)[^5]/리스크 기준(3~5개)으로 표 비교하고, 최종 추천 1가지와 트레이드오프[^3]를 설명해줘"

### 자주하는 실수
기준이 모호하거나 서로 다른 범주를 섞으면 비교가 흐려집니다.

### 더 좋게
기준은 3~5개로 제한하고, 가중치가 있다면 미리 적어주세요.

## 2.5 맥락 의존형 질문 (Context-dependent)
현실 제약(시간·인력·기술·정책)을 반영해 ‘실행 가능한’ 답을 유도합니다.

### 언제 유용한가
우리 환경 맞춤 해법, 정책 준수, 제한된 리소스 내 계획 수립

### 언제 안 쓰나
환경/버전/제약 정보가 전혀 없는 경우(먼저 개방형/탐색형으로 정보 수집).

### 예시
"Python 3.10, MacOS, 인터넷 제한 환경에서 주 2시간만 투입 가능할 때, 데이터 정제 자동화의 최소 단계와 일정 제안"
...
### 자주하는 실수
제약이 빠지면 책상머리 설계가 됩니다. 환경 정보를 맨 앞에 배치하세요.

### 더 좋게
전제(가정)를 명시하게 하여 오해를 줄이세요. 확실하지 않은 부분은 ‘추가 확인 필요’로 표시하게 합니다.

## 2.6 메타 질문 (Meta-questions)
질문을 더 나은 질문으로 재구성해 다음 질의의 품질을 끌어올립니다.

### 언제 유용한가
문제 정의가 모호할 때, 질문이 길어질 때, 이해관계자가 많을 때

### 언제 안 쓰나
이미 잘 정제된 질문이고 바로 답이 필요한 경우(메타 대신 본질 질문으로 진행).

### 예시
"내 질문을 체크리스트 형식으로 재구성하고, 평가 기준을 추가해줘"
...
### 이렇게 적어보세요
"아래 질문을 더 효과적으로 만들기 위해 누락/모호/과잉 요소를 지적하고, 개선안과 성공 기준을 제시해줘. 개선 전/후를 나란히 보여줘"

### 자주하는 실수
개선 없이 재진술만 하는 경우가 있습니다. 반드시 ‘개선 전/후 비교’를 요구하세요.

### 더 좋게
‘목적→대상→범위→형식→성공 기준’ 순서로 재구성하게 하세요.
