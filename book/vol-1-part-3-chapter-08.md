...
측정 전제·단위·방법
- 전제: 동일 작업 정의/데이터, 동일 인스트럭션(역할·입/출력·제약) 고정, 사전·후처리 없음 기준
- 단위: 품질=1–10 주관 평가(동일 평가자), 자원=토큰/비용/메시지 수(가용 지표), 속도=TTFT/전체응답시간(초)
- 방법: 각 작업 3회 이상 반복, 평균±표준편차 기록, 이상치 제거 기준 명시(예: 3σ)
- 통제: 네트워크 상태와 모델 버전 고정, 캐시/컨텍스트 길이 동일화
...
## 다음 장 미리보기: 9장. 인스트럭션의 평가와 검증

성능 최적화 전략을 배웠다면, 이제 그것이 정말 효과가 있는지 어떻게 확인할까요? 9장에서는 '감'이 아닌 명확한 기준으로 인스트럭션을 평가하는 실용적인 방법을 배웁니다.

**9장에서 배울 핵심 내용:**
- **나만의 평가 지표 만들기**: 재작업 비율, 만족도 점수, 아이디어 채택률 등 객관적인 기준 설정
- **나만의 A/B 테스트**: 두 가지 프롬프트를 체계적으로 비교하여 더 나은 것을 찾는 방법
- **오답 노트**: 실수를 기록하고 개선하면서 회귀(이전 문제의 재발)를 방지하는 전략
- **평가 에이전트**: AI를 심판으로 활용하여 자동화된 품질 검증 시스템 구축하기

성능 최적화가 '어떻게 효율을 높일까'에 대한 답이라면, 평가와 검증은 '그것이 정말 효과가 있는지 어떻게 확신할까'에 대한 답입니다. 지속적으로 발전하는 AI 협업 시스템을 만들고 싶다면, 9장의 평가 방법론이 필수적입니다.
