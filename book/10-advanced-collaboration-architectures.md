...
이 장은 **고급 아키텍처**를 다룹니다. 10장을 시작하기 전에, 다음을 먼저 이해해야 합니다:

### 7장에서 배운 것
- **1-2개 에이전트** 협업
- 파이프라인, 생성-검증, 라우팅 패턴
- `workflow.yaml`로 워크플로우 정의
- 평면적 파일 구조

### 7.5장에서 배운 것 (권장)
7.5장을 학습하지 않았다면, 10장으로 바로 오기 전에 [7.5장 중간 규모 협업과 파일 기반 관리](07-5-intermediate-collaboration.md)를 먼저 읽어보세요. 7.5장에서는 다음을 다룹니다:
- **3-4개 에이전트** 팀 협업
- 디렉토리 구조 설계 (`/tasks`, `/outputs`, `/shared`)
- `status.json`을 사용한 상태 추적
- JSON 기반 산출물

### 10장에서 다룰 범위
10장은 **6개 이상의 에이전트**가 필요한 복잡한 시스템을 다룹니다:
- ✅ 메타/아키텍트/워커 **계층 구조**
- ✅ 에이전트가 다른 에이전트를 **동적으로 생성**
- ✅ `/agents`, `/workflows`, `/jobs` 분리된 **다층 디렉토리**
- ✅ `_job_log.json`으로 **여러 작업 관리**

**판단 기준**: 다음 신호가 보이면 10장이 필요합니다:
- 에이전트가 5개 이상 필요
- "누가 어떤 에이전트를 만들 것인가?"가 동적으로 결정됨
- 여러 프로젝트가 동시에 진행됨

**참고**: 1-2개 에이전트로 충분하다면 7장으로, 3-4개면 7.5장으로 돌아가세요. 모든 프로젝트에 10장의 복잡한 구조가 필요한 것은 아닙니다.
