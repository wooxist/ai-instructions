...
> **용어 정의: 메타 에이전트 vs. 최상위 아키텍트**
> 이 책에서 '최상위 아키텍트(Top Architect)'는 여러 아키텍트 에이전트를 지휘하는 시스템 내 최고 운영 관리자를 의미합니다. 반면, '메타 에이전트(Meta Agent)'라는 용어는 에이전트 조직 자체를 설계하고 시스템의 구조를 정의하는, 시스템 외부의 유일한 '설계자' 역할을 지칭하기 위해 사용되므로, 둘을 명확히 구분해야 합니다.

가장 대표적인 운영 모델이 바로 **'최상위 아키텍트 - 아키텍트 - 워커'** 3단계 계층 구조입니다.

```mermaid
graph TD;
    subgraph "최상위 관리 계층"
        TopArchitect["최상위 아키텍트 (총괄 관리자)"]
    end
    ...
    TopArchitect -- "프로젝트 A 할당" --> ArchitectAgentA
    TopArchitect -- "프로젝트 B 할당" --> ArchitectAgentB
    ...
    ArchitectAgentA -- "프로젝트 A 결과 보고" --> TopArchitect
    ArchitectAgentB -- "프로젝트 B 결과 보고" --> TopArchitect
```
...
-   **책임**:
    ...
    -   전체 워크플로우가 완료되면, 최종 결과를 종합하여 상위 계층(최상위 아키텍트)에 보고합니다.
...
### 10.3.4 최상위 아키텍트 (Top Architect): 시스템 총괄 관리자
...
-   **책임**:
    ...
    -   시스템의 전반적인 성능 지표(예: 전체 작업 성공률, 평균 처리 시간)를 분석하고, 필요시 시스템 개선을 지시합니다.
