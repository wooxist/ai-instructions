...
> **용어 정의: 아키텍트와 워커**
> - **아키텍트 에이전트(Architect Agent):** 워크플로우를 설계하고, 여러 워커 에이전트를 지휘하며 프로젝트 전체를 감독하는 '프로젝트 매니저'입니다.
> - **워커 에이전트(Worker Agent):** 아키텍트의 지시를 받아 특정 실무를 수행하는 '전문 실무자'입니다.

가장 대표적인 운영 모델이 바로 **'아키텍트 - 워커'** 2단계 계층 구조입니다.

```mermaid
graph TD;
    subgraph "워크플로우 설계 및 감독 계층"
        ArchitectAgent["아키텍트 에이전트 (프로젝트 매니저)"]
    end
    ...
    ArchitectAgent -- "태스크 1 지시" --> WorkerAgent1
    ArchitectAgent -- "태스크 2 지시" --> WorkerAgent2
    ArchitectAgent -- "태스크 3 지시" --> WorkerAgent3

    WorkerAgent1 -- "결과 보고" --> ArchitectAgent
    WorkerAgent2 -- "결과 보고" --> ArchitectAgent
    WorkerAgent3 -- "결과 보고" --> ArchitectAgent
```
...
-   **책임**:
    ...
    -   전체 워크플로우가 완료되면, 최종 결과를 종합하여 사용자에게 보고합니다.
