...
# 처리 방법
1. 글의 구조는 서론, 본론(RAG의 원리, 장점), 결론 순서를 따른다.
2. 어려운 기술 용어는 반드시 쉬운 비유를 들어 설명한다.

# 제약 조건
- 반드시 아래 제공된 `[참고 자료]` 섹션의 내용만을 기반으로 글을 작성해야 하며, 외부 지식이나 개인적인 의견을 포함해서는 안 된다.
...
#### 설계 분석
- **메타 원칙 (4장):**
  - **컨텍스트 명시성** 및 **투명성·추적성**: `#제약 조건`을 통해 AI가 사용할 정보 소스를 `#참고 자료`로 명시적으로 한정합니다. 이는 AI가 추측하는 것을 막고(컨텍스트 명시성), 결과물의 모든 근거를 추적할 수 있게(투명성·추적성) 만듭니다.
  - **산출물 중심**: `#출력 형식`에서 결과물의 사양을 명확히 정의합니다.
- **에이전트 설계 (5장):** '10년차 테크니컬 라이터'라는 전문적인 **페르소나**를 부여하고, `#제약 조건`을 통해 행동(정보 출처 제한)의 경계를 명확히 설정합니다.
- **입/출력 설계 (6장):** `#참고 자료`가 명시적인 **입력** 데이터가 되며, `#출력 형식`이 **출력** 명세가 됩니다.
