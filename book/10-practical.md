...
#### 최종 인스트럭션 예시
```markdown
# 워크플로우: 아이디어 → 트윗 변환 및 최적화

1. **AGENT 1: 아이디어 확장 에이전트**
   - **입력:** 한 문장의 아이디어 (예: "AI를 활용한 생산성 팁")
   - **역할:** 창의적인 콘텐츠 전략가
   - **태스크:**
     1. 입력된 아이디어를 바탕으로, 사람들의 흥미를 끌 만한 3가지 다른 각도의 접근법을 제안한다.
     2. 각 접근법은 (1) 질문형, (2) 정보 제공형, (3) 논란 유발형 중 하나여야 한다.
   - **출력:** 3가지 접근법 목록 (JSON 형식)
     - 예시 출력:
       {
         "approaches": [
           {"type": "질문형", "content": "여러분의 최애 AI 생산성 도구는 무엇인가요? 댓글로 공유해주세요!"},
           {"type": "정보 제공형", "content": "AI 회의록 요약 도구를 사용하면 회의 시간을 30% 절약할 수 있습니다."},
           {"type": "논란 유발형", "content": "AI가 인간의 모든 업무를 대체할 것이라는 주장은 과장되었습니다. 진짜 위협은 다른 곳에 있습니다."}
         ]
       }

2. **AGENT 2: 트윗 작성 에이전트**
   - **입력:** 사용자가 선택한 1가지 접근법 (예: "정보 제공형" 접근법)
   - **역할:** 재치 있는 소셜 미디어 전문가
   - **태스크:**
     1. 선택된 접근법을 140자 이내의 간결하고 흡입력 있는 트윗 문장으로 다듬는다.
     2. 문맥에 맞는 이모지를 1~2개 포함한다.
   - **출력:** 최종 트윗 문장 (문자열)
     - 예시 출력: "🤖 AI 회의록 요약 도구, 써보셨나요? 잘만 활용하면 회의 시간을 무려 30%나 아낄 수 있다는 사실! #생산성팁 #업무자동화"

3. **AGENT 3: 해시태그 추천 에이전트**
   - **입력:** AGENT 2가 작성한 트윗 문장
   - **역할:** 트렌드 분석가
   - **태스크:**
     1. 트윗 내용과 관련된 해시태그 5개를 추천한다.
     2. 추천 목록에는 일반적인 키워드와 최근 트렌드를 반영한 키워드가 섞여 있어야 한다.
   - **출력:** 추천 해시태그 목록 (JSON 배열)
     - 예시 출력: `["#AI", "#인공지능", "#생산성", "#업무효율", "#스마트워크"]`
```

#### 설계 분석
- **메타 원칙 (4장):** **SoC(관심사 분리)** 원칙을 적용해 **각 에이전트가 한 가지 일에만 집중하게 만들었습니다.** 아이디어 생성, 문장 다듬기, 키워드 추천을 분리하면 각 단계의 전문성이 높아지고, 나중에 특정 기능만 수정하거나 개선하기 용이해집니다. 또한 **점진적 개선** 원칙에 따라, 추상적인 아이디어가 구체적인 트윗과 해시태그로 단계적으로 발전하므로 복잡한 문제를 체계적으로 해결할 수 있습니다.
...
#### 설계 분석
- **메타 원칙 (4장):** 이 워크플로우의 가장 큰 특징은 **피드백 루프**를 통해 스스로 결과물을 개선한다는 점입니다. 이를 구현하기 위해 먼저 **SoC** 원칙으로 '코드를 짜는 일(생성)'과 '코드를 검토하는 일(검증)'의 책임을 명확히 분리했습니다. 덕분에 각 에이전트는 자신의 전문성에만 집중할 수 있습니다. 또한, 리뷰 기준을 구체적인 항목으로 나눈 것은 **원자성** 원칙을 따른 것으로, 무엇이 잘못되었는지 명확하게 지적하고 수정하도록 돕습니다.
...
- **워크플로우 설계 (7장):** 결론적으로 이 설계는 **'생성-검증' 패턴**과 **'분기 패턴'**을 결합하여, **스스로 결과물을 평가하고 문제가 발견되면 알아서 수정하는 지능형 워크플로우**를 구현합니다. 이는 한번 실행하고 끝나는 단순 자동화를 넘어, 정해진 품질 기준을 만족할 때까지 작업을 반복하는 한 단계 높은 수준의 자동화 방식입니다.

> **실용적인 팁: 워크플로우를 '언어'처럼 설계하기**
> 지금 보신 피드백 루프는 마치 이 워크플로우만을 위한 작은 **'도메인 특화 언어(DSL)'**를 설계하는 것과 같습니다. '통과' 또는 '수정 필요' 같은 상태를 `JSON`으로 정의하고, `IF/ELSE` 같은 조건으로 흐름을 제어하는 규칙을 만드는 이 접근 방식은 워크플로우의 가독성과 유지보수성을 크게 향상시키는 강력한 방법입니다.
> 
> 이처럼 인스트럭션을 코드로 바라보고 설계하는 더 깊이 있는 방법론은 **[12장. 고급 워크플로우: DSL처럼 인스트럭션 설계하기](12-workflow-as-code.md)** 에서 자세히 다룹니다.
...
[^1]: **규모와 복잡도 매트릭스 (Scale & Complexity Matrix):** 4장에서 소개된, 인스트럭션 시스템의 설계 깊이를 결정하는 2차원 프레임워크. 에이전트 규모(단일/다중/조직)와 작업 복잡도(간단/표준/복잡)의 조합으로 총 9가지 상황을 정의하며, 각 상황에 적합한 설계 접근법을 제시한다.

[^2]: **파이프라인 패턴 (Pipeline Pattern):** 여러 처리 단계를 순차적으로 연결하여, 각 단계가 이전 단계의 출력을 입력으로 받아 작업을 수행하는 구조. A → B → C 형태로 표현되며, 데이터 처리 및 변환 작업의 가장 기본적인 워크플로우 패턴이다.

[^3]: **생성-검증 패턴 (Generate-and-Validate Pattern):** 하나의 에이전트가 결과물을 생성하면, 다른 에이전트가 이를 검증하고 수정하는 2단계 워크플로우. 할루시네이션 방지와 품질 향상에 효과적이며, 7장에서 소개된 기본 협력 패턴 중 하나이다.

[^4]: **계층적 협력 시스템 (Hierarchical Agent Collaboration):** 에이전트에게 메타-보스-비서-매니저-워커와 같은 명확한 위계질서를 부여하여, 복잡한 문제를 체계적으로 분해하고 각 계층이 자신의 추상화 수준에서 최적의 의사결정을 내리도록 하는 시스템 아키텍처. 7장 참조.

[^5]: **수평적 협력 시스템 (Horizontal Agent Collaboration):** 계층 없이 동등한 위치의 전문가 에이전트들이 병렬적으로 협력하여 문제를 해결하는 시스템 아키텍처. 전문가 패널, 동료 검토, 라운드 로빈 등의 패턴을 포함하며, 의사결정의 분산과 전문성 보장에 유리하다. 7장 참조.

[^6]: **Human-in-the-Loop:** AI 시스템의 주요 의사결정 지점에 인간의 검토와 승인을 포함시켜, 시스템의 책임성과 신뢰성을 보장하는 설계 원칙. 특히 법률, 재무, 의료 등 고위험 영역에서 필수적이다.