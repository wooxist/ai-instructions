...
**둘째, 에이전트가 사용할 지식의 출처를 한정합니다.** 이는 마치 가족 여행 계획을 짤 때, 인터넷의 무수한 정보 대신 '우리 가족의 공유 캘린더'와 '예약된 항공권/숙소 정보'만을 '유일한 진실의 공급원'으로 삼는 것과 같습니다. 에이전트에게 "이 논문들의 내용만으로 블로그 글을 작성하라"고 지시하면, AI는 방대한 일반 지식 대신 우리가 제공한 특정 문서만을 참조하게 됩니다. 이 방식은 결과물의 사실 기반을 보장하고(grounding) 모든 주장의 근거를 추적할 수 있게 하여(traceability) 할루시네이션을 방지하는 데 결정적인 역할을 합니다. 도구·검색 설계 관점의 구현은 11장. 도구와 프레임워크에서, 역할·제약 관점의 구체화는 5장. 역할과 제약 설계에서 다룹니다.

이 두 방식은 모두 **'진실'의 출처를 중앙에서 통제하여 시스템 전체의 신뢰도를 높인다**는 동일한 목표를 공유합니다.

다음으로 **SoC(Separation of Concerns)[^2]**, 즉 '관심사 분리' 원칙입니다. 이는 마치 주방에서 한 사람은 재료 손질만, 다른 사람은 요리만, 또 다른 사람은 설거지만 담당하는 것과 같습니다. 각자가 자신의 역할에만 집중할 때 전체 프로세스가 더 효율적이고 실수가 줄어듭니다. 인스트럭션 설계에서도 AI의 역할을 정의하는 부분, 처리 방법을 지시하는 부분, 결과물의 형식을 지정하는 부분을 분리하면, 각 부분을 독립적으로 수정하고 재사용하기가 훨씬 수월해집니다. 이 책의 5장이 '역할과 제약'을, 6장이 '입력과 출력'을 다루는 것도 이 원칙을 따른 것입니다.
