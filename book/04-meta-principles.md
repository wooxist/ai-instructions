...
**단일 에이전트 (1개)**
- 하나의 AI가 독립적으로 작업을 완료하는 경우입니다.
- 예: ChatGPT로 이메일 요약, Claude로 문서 번역, 단일 보고서 작성
- 특징: 에이전트 간 데이터 전달이나 역할 분담이 없음

**다중 에이전트 (2-5개)**
- 여러 AI 에이전트가 역할을 나누어 협업하는 경우입니다.
...
**조직 표준 (6개 이상, 다수 사용자)**
- 다수의 에이전트를 여러 사람이 공유하여 사용하는 경우입니다.
- 예: 기업의 표준 인스트럭션 라이브러리, 부서별 특화 에이전트 세트
- 특징: 일관성, 유지보수성, 버전 관리가 매우 중요하며, 표준화된 인터페이스가 필요

중요한 점은 **1인이라도 여러 AI 에이전트를 활용한다면 "다중 에이전트" 수준의 원칙이 필요**하다는 것입니다. 예를 들어, 1인 스타트업 창업자가 기획·개발·마케팅 각 영역에 특화된 에이전트를 운영한다면, 에이전트 간 일관된 용어(SSOT), 명확한 역할 분리(SoC), 빠짐없는 커버리지(MECE)가 필수입니다.
