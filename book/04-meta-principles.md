...
**둘째, 에이전트가 사용할 지식의 출처를 한정합니다.** 이는 에이전트가 작업을 수행할 때 참조해야 할 '세상'을 우리가 제공하는 특정 문서나 데이터로 제한하는 것입니다. 예를 들어, "이 논문들의 내용만으로 블로그 글을 작성하라"고 지시하는 것은 AI가 가진 방대한 일반 지식 대신, 우리가 제공한 논문들을 '유일한 진실의 공급원'으로 삼게 만듭니다. 이 방식은 결과물의 사실 기반을 보장하고(grounding) 모든 주장의 근거를 추적할 수 있게 하여(traceability) 할루시네이션을 방지하는 데 결정적인 역할을 합니다.
