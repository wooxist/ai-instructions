...
**첫째, 인스트럭션 설계 자체의 중복을 제거합니다.** 여러 에이전트가 공통으로 사용하는 규칙, 템플릿, 출력 스키마 등을 별도의 파일로 분리하고, 각 인스트럭션이 이를 참조하게 만드는 방식입니다. 이는 소프트웨어 개발의 **DRY(Don't Repeat Yourself)** 원칙과 같습니다. 예를 들어, 여러 에이전트가 동일한 JSON 출력 형식을 사용한다면, 각 프롬프트에 스키마를 복사하는 대신 중앙의 `schema.json` 파일 하나를 참조하게 만들어야 합니다.

**둘째, 에이전트가 사용할 지식의 출처를 한정합니다.** 이는 에이전트가 작업을 수행할 때 참조해야 할 '세상'을 우리가 제공하는 특정 문서나 데이터로 제한하는 것입니다. 예를 들어, "이 논문들의 내용만으로 블로그 글을 작성하라"고 지시하는 것은 AI가 가진 방대한 일반 지식 대신, 우리가 제공한 논문들을 '유일한 진실의 공급원'으로 삼게 만듭니다. 이 방식은 결과물의 사실 기반을 보장하고(grounding) 모든 주장의 근거를 추적할 수 있게 하여(traceability) 할루시네이션을 방지하는 데 결정적인 역할을 합니다. 도구·검색 설계 관점의 구현은 [11장. 도구와 프레임워크](11-tools.md)에서, 역할·제약 관점의 구체화는 [5장. 역할과 제약 설계](05-agent-constraints.md)에서 다룹니다.
