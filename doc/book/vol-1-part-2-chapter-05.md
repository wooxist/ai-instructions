...
- **지식 출처 한정(SSOT) 적용:** 4장의 SSOT 중 "지식 출처 한정" 원칙을 에이전트 **제약**으로 구체화합니다. 허용된 자료원(코퍼스/문서 집합)만 사용하도록 명시하고, 결과에는 반드시 **출처 식별자와 근거 인용(citations)**을 포함하게 합니다. 도구·검색 구현 방법은 [12장. 도구와 플러그인 활용](12-tools.md) 참고.
...
flowchart TD
  S([시작]) --> Q1{컨텍스트 윈도우<br/>초과 위험?}
  Q1 -- 예 --> A1[데이터/단계 기준으로<br/>역할 분할]
  Q1 -- 아니오 --> Q2{단일 책임(SRP)<br/>충족?}
  Q2 -- 아니오 --> A2[책임 축소/분리]
  Q2 -- 예 --> Q3{응집도<br/>높은가?}
  Q3 -- 아니오 --> A3[관련성 낮은 책임 분리]
  Q3 -- 예 --> Q4{재사용성<br/>높은가?}
  Q4 -- 예 --> A4[별도 에이전트로<br/>격리]
  Q4 -- 아니오 --> Q5{보안/권한/검증<br/>분리가 필요한가?}
  Q5 -- 예 --> A5[검증/도구/권한 전담<br/>에이전트 분리]
  Q5 -- 아니오 --> A6[현 구조 유지]
...
## 👁️ 다음 장 미리보기: 6장. 입력과 출력 설계

### 역할을 정했다면, 이제 ‘재료’와 ‘완성품’을 명확히 하세요

5장에서 우리는 ‘역할’과 ‘제약’으로 에이전트를 설계하는 법을 배웠습니다. 하지만 아무리 민능한 전문가라도, 작업에 필요한 **‘재료(입력)’**가 잘못되거나 만들어야 할 **‘완성품(출력)’**에 대한 정의가 모호하다면 제대로 된 결과물을 낼 수 없습니다.

**6장에서는 다음을 배웁니다:**

- **"Garbage In, Garbage Out"**: 모호한 입력이 어떻게 나쁨 결과로 이어지는지
- **입력 명세**: 에이전트가 사용할 데이터와 컨텍스트를 명확히 정의하는 법
- **출력 명세**: JSON Schema, Few-shot 예시로 예측 가능하고 일관된 결과물 확보하기
- **의도 확인 루프**: 모호한 요구사항을 AI가 질문으로 명확히 하게 만드는 기법

**구체적인 예시:**
‘번역 에이전트’를 만들었다고 가정해봅시다. 5장에서 역할과 제약을 정의했지만, 여전히 다음과 같은 문제가 남아있습니다:

- 어떤 포맷으로 결과를 받아야 할까요? (단순 문자열? JSON?)
- 사용자가 "시적으로 번역해줘"라고 하면 어떻게 처리할까요?
- 번역 품질을 어떻게 검증할까요?

6장에서는 이런 문제들을 **산출물 중심(Output-Driven)** 접근법으로 해결하는 방법을 배웁니다. 원하는 결과물을 먼저 명확히 정의하고, 그를 위해 필요한 입력을 역으로 설계하는 실용적인 기법들을 익히게 됩니다.

**준비되셨다면, [6장: 입력과 출력 설계](06-input-output.md)로 이동하세요!**
...
**히트:** 5.2절의 4가지 예시 에이전트(분류, 핵심 추출, 초안 작성, 검증)를 참고하세요.
...
4. **최종 설계**: 에이전트 목록과 각의 역할, 제약, 페르소나 작성
...
### 이 장을 완료하셨다면 다음을 확인하세요:
- [ ] 단일 책임 원칙(SRP)에 따라 에이전트 경계를 정의할 수 있다
- [ ] 역할, 제약, 페르소나, 안전장치(예: 허용 자료원, 인용 요구)를 명시할 수 있다
- [ ] 보안 취약점(프롬프트 인젝션 등)에 대한 제약을 인스트럭션에 반영할 수 있다

### 실습 과제
1. 본인의 업무 1개를 2~4개의 워커 에이전트로 분해하고, 각 에이전트의 ‘역할/제약/출력’을 표로 정리하세요.
2. 한 에이전트에 SSOT 기반 ‘지식 출처 한정’과 ‘근거 인용’ 규칙을 추가하세요.
