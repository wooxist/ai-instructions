...
## 4.1 AI 협업의 4가지 제약 조건

좋은 인스트럭션을 만들기 위한 설계 원칙들은 다음 장에서 자세히 배울 예정입니다. 하지만 그 원칙들이 왜 필요한지 더 깊이 이해하려면, 먼저 AI와 협업할 때 마주치는 현실적인 제약들을 알아볼 필요가 있습니다.
...
### 4.1.1 제약 1: Context Window 한계와 Lost in the Middle

AI가 한 번에 기억하고 처리할 수 있는 정보량에는 한계가 있습니다(Context Window). 이 한계를 넘어서면 초반의 정보가 잘려나가고, 한계 내에 있어도 긴 지시에서는 중간 부분의 중요한 규칙을 잘 활용하지 못하는 경향이 있습니다(Lost in the Middle)[^1].

**문제 상황:**
- "이 문서를 요약하되, 긍정적인 표현만 사용하고, 각 문단은 3문장 이내로, 전문 용어는 쉬운 말로..."
- → 지시 항목이 많아질수록 AI는 중간에 제시한 규칙들을 놓치기 쉽습니다
...
### 4.1.2 제약 2: 할루시네이션 (Hallucination)
...
### 4.1.4 제약 4: 유지보수 어려움 (Maintenance Difficulty)
...
1. **Context Window 한계와 Lost in the Middle**
   - 작업을 단계별로 분할
   - 핵심 규칙을 구조화
...
이것이 바로 다음 5장에서 배울 **관심사 분리(SoC)** 원칙을 AI 인스트럭션에 적용하는 것입니다.
...
4. 유지보수 어려움 (Maintenance Difficulty)
...
## 4.3 제약에서 원칙으로: 더 큰 그림 보기

우리는 이 장에서 AI의 4가지 제약과 각각의 실용적 해결 방법을 배웠습니다.

이제 중요한 통찰이 있습니다:
**이런 해결 방법들은 사실 더 큰 설계 원칙의 구체적 적용입니다.**

예를 들어:
- "작업을 단계별로 분할" → **원자성(Atomicity)** 원칙의 적용
- "역할별로 프롬프트 분리" → **관심사 분리(SoC)** 원칙의 적용  
- "공통 규칙을 한 곳에" → **단일 진실 공급원(SSOT)** 원칙의 적용
- "출력 템플릿 제공" → **산출물 중심(Output-Driven)** 원칙의 적용

이런 개별 해결책들이 왜 효과적인지, 어떤 원리에 기반하는지 이해한다면,
**새로운 제약을 만나더라도 스스로 해결책을 설계할 수 있게 됩니다.**

다음 장에서는 이러한 개별 해결책들을 아우르는 **11가지 시스템 설계 원칙**을 체계적으로 배웁니다.
원칙을 이해하면 어떤 제약이든 체계적으로 해결할 수 있는 설계 역량을 갖추게 됩니다.
...
[^1]: **Lost in the Middle & Context Window:** AI 모델은 한 번에 처리할 수 있는 정보량(Context Window)에 물리적 한계가 있습니다. 예를 들어 Claude 3.5 Sonnet은 약 200,000 토큰(약 15만 단어)을 처리할 수 있지만, 이 한계를 넘어서면 초반 정보가 자동으로 잘려나갑니다. 더 중요한 점은, Context Window 내에 있는 정보라도 "Lost in the Middle" 현상이 발생한다는 것입니다. Liu et al. (2023)의 연구에 따르면, AI 모델은 긴 입력에서 시작 부분과 끝 부분의 정보는 잘 활용하지만, 중간에 위치한 정보는 상대적으로 덜 활용하는 경향을 보입니다. 이는 Attention 메커니즘이 입력의 처음과 끝에 더 많은 가중치를 부여하기 때문입니다. 따라서 긴 프롬프트에서 중간에 중요한 규칙을 배치하면, 해당 규칙이 무시되거나 제대로 적용되지 않을 수 있습니다.
