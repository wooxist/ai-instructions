...
예를 들어, '명확성'과 '구체성'은 [산출물 중심(Output-Driven)](glossary.md#산출물-중심-output-driven) 메타 원칙의 구체적인 실행 방법이며, '단계성'은 복잡한 문제를 나누어 해결하는 **원자성(Atomicity)** 및 [관심사 분리(SoC)](glossary.md#soc-separation-of-concerns)와 깊이 관련됩니다. 즉, 3장의 원칙들이 '전술'이라면, 4장의 메타 원칙들은 이 전술들을 이끄는 '전략'입니다.
...
## 다음 장 미리보기: 5장. 역할(Agent)과 제약(Constraint) 설계

4장에서 배운 메타 원칙들은 좋은 인스트럭션 설계의 '지도'입니다. 그렇다면 이 지도를 실제 업무에 어떻게 적용할까요? **5장에서는 이 원칙들을 실제 '에이전트' 설계로 구현하는 방법**을 배웁니다.

단일 프롬프트가 가진 본질적 한계(컨텍스트 윈도우, 할루시네이션, 비일관성)를 극복하고, 복잡한 업무를 자동화하기 위해서는 **명확한 역할과 책임을 가진 전문가 '에이전트'**로 분할해야 합니다. 5장에서는 다음을 다룹니다:

- **왜 에이전트를 설계해야 하는가**: AI의 한계를 극복하고 체계적 자동화의 기회를 만드는 방법
- **에이전트 분할 방법론**: 관심사 분리(SoC)와 단일 책임 원칙(SRP)을 실제 설계에 적용하는 법
- **에이전트 범위 설계**: 컨텍스트 윈도우 크기 계산, 인스트럭션-데이터 비율(20-30%), 전문성·재사용성·응집도 기준으로 분리 결정하기
- **역할·제약·페르소나 설계**: 할루시네이션 방지, 일관성 확보, 모듈화를 위한 구체적 기법

4장에서 배운 SSOT, SoC, MECE, 산출물 중심 같은 추상적 원칙들이, 5장에서는 '근거 기반 책임', '검증 에이전트', '명시적 제약' 같은 구체적인 설계 요소로 변환됩니다. 이제 인스트럭션 설계의 **구체적인 실행 방법론**을 익힐 차례입니다.
