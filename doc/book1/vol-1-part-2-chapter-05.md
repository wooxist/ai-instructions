# 5장. 에이전트(Agent)와 제약(Constraint) 설계
...
우리는 단일 프롬프트의 5가지 한계(컨텍스트 윈도우, 할루시네이션, 비일관성, 유지보수, 보안)와 이를 극복했을 때의 3가지 기회(모듈화, 효율성, 명확한 책임)를 배웠습니다.

그렇다면 이제 중요한 질문이 생깁니다.

"이 한계들을 실제로 어떻게 해결하나요? 구체적인 방법이 있나요?"

바로 이 지점에서, 4장에서 배운 메타 원칙들이 실전에 투입됩니다.
...
1. **단일 책임 원칙(SRP)**: 하나의 에이전트는 하나의 일만 합니다. 마치 회사에서 각 팀원이 자기 업무만 맡듯이요.

2. **관심사 분리(SoC)**: 복잡한 작업을 여러 에이전트로 나누기. '고객 리뷰 분석'을 '분류', '추출', '보고서 작성' 등으로 쪼개는 거죠.

3. **근거 기반 책임**: AI가 지어낸 내용이 아닌, 제공된 자료에서만 답하기. 모든 주장에 출처를 붙이는 겁니다.

나머지 개념(MECE, SSOT, RAG 등)은 이 3가지를 보완하는 도구예요. 처음에는 위 3개만 확실히 이해하세요!

---

'에이전트'는 단순히 프롬프트를 나누는 것을 넘어, 특정 역할, 책임, 제약 조건을 가진 독립적인 실행 단위입니다. 예를 들어, '고객 리뷰 분석'이라는 큰 작업은 다음과 같은 [에이전트(Agent)](glossary.md#에이전트-agent)들의 협력으로 수행될 수 있습니다.
...
할루시네이션을 줄이고 신뢰도를 높이기 위해, 에이전트에게 '책임'을 부여하고 별도의 에이전트로 분리해야 합니다. 이는 마치 언론사의 '기자'와 '팩트 체커'처럼 역할을 나누는 것과 같습니다. 이는 4장의 **투명성 및 추적 가능성(Transparency & Traceability)** 원칙과 연결됩니다.

- **근거 기반 책임 부여:** 모든 '생성' 관련 에이전트에게 "제공된 문서나 데이터 내에서만 정보를 찾아야 하며, 모든 주장은 반드시 출처를 명시해야 한다"는 핵심 **책임**을 부여하고, "추측은 엄격히 금지된다"는 **제약**을 설정합니다.
- **지식 출처 한정(SSOT) 적용:** 4장의 SSOT 중 "지식 출처 한정" 원칙을 에이전트 **제약**으로 구체화합니다. 허용된 자료원(코퍼스/문서 집합)만 사용하도록 명시하고, 결과에는 반드시 **출처 식별자와 근거 인용(citations)**을 포함하게 합니다.
...
"내 에이전트를 얼마나 크게, 또는 작게 만들어야 하는가?" 이 질문에 답하기 위해, 설계 원칙에 따라 에이전트의 범위를 결정하는 방법을 알아봅니다.

### 5.3.1 설계 원칙에 따른 에이전트 분리 결정

에이전트를 설계할 때 각 에이전트가 맡을 책임을 명확히 하기 위해 다음 세 가지 관점을 고려해보면 좋습니다.
...
| 항목 | 토큰 수 | 비율 | 설명 |
|------|---------|------|------|
| 전체 윈도우 | 128,000 | 100% | 모델의 총 용량 |
| **인스트럭션** | **25,600** | **20%** | 에이전트 지시사항 |
| 입력 데이터 | 76,800 | 60% | 처리할 문서/데이터 |
| 출력 데이터 | 25,600 | 20% | AI가 생성할 결과 |

💡 **한글 토큰 계산 팁**
- 한글 1글자 ≈ 1.5~2 토큰
- 예: "안녕하세요" (5글자) ≈ 7~10 토큰
- 예: 5,000자 문서 ≈ 7,500~10,000 토큰
- 정확한 계산: tiktoken 라이브러리 사용 (과제 2 참고)
...
복잡한 업무를 언제, 어떻게 에이전트로 분할할지 빠르게 판단할 수 있도록 의사결정 트리를 제공합니다.

### 트리 사용법: 단계별로 따라가기

💡 **예시: "고객 리뷰 분석 보고서 작성" 작업 분할하기**

**1단계: 컨텍스트 윈도우 체크**
- 시작 → Q1: 컨텍스트 윈도우 초과 위험?
- 리뷰 1,000개 xd7 평균 200토큰 = 200k 토큰
- GPT-4 (128k) 초과! → **예** → **A1: 역할 분할**

**2단계: 분할 후 각 에이전트 체크**
- "분류 에이전트" → Q2: 단일 책임? → **예** (분류만 담당)
- Q3: 응집도? → **예** (분류라는 한 가지 목적)
- Q4: 재사용성? → **예** (다른 분석에서도 쓸 수 있음)
- → **A4: 별도 에이전트로 격리**

**3단계: 모든 에이전트에 반복**
이런 식으로 모든 에이전트(추출, 초안, 검증)에 대해 트리를 따라가면서 확인합니다.
...
flowchart TD
    S([시작])
    Q1{"컨텍스트 윈도우<br/>초과 위험?"}
    A1["데이터/단계 기준으로<br/>역할 분할"]
    Q2{"단일 책임<br/>충족?"}
    A2["책임<br/>축소/분리"]
    Q3{"응집도<br/>높은가?"}
    A3["관련성 낮은<br/>책임 분리"]
    Q4{"재사용성<br/>높은가?"}
    A4["별도 에이전트로<br/>격리"]
    Q5{"보안/권한/검증<br/>분리 필요?"}
    A5["검증/도구/권한<br/>전담 에이전트 분리"]
    A6["현 구조<br/>유지"]
    ...
    classDef startEnd fill:#e9ecef,stroke:#495057,stroke-width:2px
    classDef question fill:#fff3cd,stroke:#856404,stroke-width:2px
    classDef action fill:#d1ecf1,stroke:#0c5460,stroke-width:2px
    classDef keep fill:#d4edda,stroke:#155724,stroke-width:2px
    
    class S,A1 startEnd
    class Q1,Q2,Q3,Q4,Q5 question
    class A2,A3,A4,A5 action
    class A6 keep
```

## 5.5 역할·제약 스니펫 (JSON/YAML)

지금까지 배운 에이전트 설계 원칙들을 실제 코드로 구현하는 방법을 알아보겠습니다. 아래는 역할과 제약을 명확히 기술한 최소 스펙 예시입니다. 6장에서는 출력 스키마(JSON Schema)까지 포함해 보다 정교한 명세를 다룹니다.
...
이 장에서는 복잡한 작업을 처리할 때 단일 프롬프트의 한계를 극복하기 위한 에이전트 설계 방법론을 배웠습니다.

**AI의 5가지 한계**를 명확히 이해했습니다: 컨텍스트 윈도우, 할루시네이션, 비일관성, 유지보수, 보안. 이 한계들은 단일 프롬프트로는 극복하기 어렵지만, **에이전트 시스템**으로 전환하면 3가지 기회(모듈화, 효율성, 명확한 책임)로 바뀝니다.

**4가지 핵심 설계 방법론**을 익혔습니다:
1. 할루시네이션 → 근거 기반 책임 + 검증 역할
2. 컨텍스트 윈도우 → 역할 분할 + 명시적 제약
3. 비일관성 → 페르소나 + 구조화된 출력
4. 유지보수 → 모듈화 + MECE

**2단계 에이전트 범위 설계**를 배웠습니다: 먼저 물리적 제약(컨텍스트 윈도우 20-30% 규칙)을 확인하고, 그 다음 설계 원칙(전문성, 재사용성, 응집도)을 적용합니다.

다음 장부터는 이렇게 설계된 에이전트가 실제로 무엇을 받아(입력) 무엇을 만들어내는지(출력)를 명확히 정의하는 방법을 배웁니다.
...
**개념 이해:**
- [ ] 단일 프롬프트의 5가지 한계(컨텍스트 윈도우, 할루시네이션, 비일관성, 유지보수, 보안)를 설명할 수 있다
- [ ] 에이전트와 프롬프트의 차이를 이해했다
- [ ] 단일 책임 원칙(SRP)과 SoC의 관계를 설명할 수 있다
- [ ] 에이전트 범위 결정 시 2단계 접근법(물리적 제약 → 설계 원칙)을 이해했다

**실습 능력:**
- [ ] 컨텍스트 윈도우 크기를 고려하여 인스트럭션-데이터 비율(20-30%)을 계산할 수 있다
- [ ] 할루시네이션을 방지하기 위해 근거 기반 책임과 검증 역할을 설계할 수 있다
- [ ] 페르소나와 구조화된 출력을 설계하여 비일관성을 해결할 수 있다
- [ ] 3가지 관점(전문성, 재사용성, 응집도)으로 에이전트 분리 여부를 결정할 수 있다

**적용 및 활용:**
- [ ] 복잡한 프롬프트를 여러 에이전트로 분할할 수 있다
- [ ] 각 에이전트에 명확한 역할, 제약, 페르소나를 부여할 수 있다
- [ ] 사용 중인 AI 모델의 컨텍스트 윈도우 크기를 확인할 수 있다

### 실습 과제

**과제 1: 에이전트 분할 연습**

다음의 복잡한 작업을 단일 책임을 가진 여러 에이전트로 분할하세요:

*원본 작업:* "고객 리뷰를 분석하여 주간 보고서를 작성하라"

1. 각 단계를 담당할 에이전트 정의 (3-5개)
2. 각 에이전트의 단일 책임 명시
3. 각 에이전트의 입력과 출력 명세

**힌트:** 5.2절의 4가지 예시 에이전트(분류, 핵심 추출, 초안 작성, 검증)를 참고하세요.

<details>
<summary>💡 예시 답변 보기</summary>

**1. 분류 에이전트**
- **단일 책임**: 고객 리뷰를 긴정/부정/문의 3가지 중 하나로 분류
- **입력**: 원본 리뷰 텍스트
- **출력**: `{category: "긴정"|"부정"|"문의", confidence: 0.0~1.0}`

**2. 핵심 추출 에이전트**
- **단일 책임**: 각 리뷰에서 핵심 단어 5개 이내 추출
- **입력**: 원본 리뷰 + 분류 결과
- **출력**: `{keywords: ["단어1", "단어2", ...]}`

**3. 초안 작성 에이전트**
- **단일 책임**: 분류와 키워드를 바탕으로 주간 보고서 초안 작성
- **입력**: 모든 리뷰의 분류 + 키워드 결과
- **출력**: 마크다운 형식의 보고서 초안

**4. 검증 에이전트**
- **단일 책임**: 초안에 할루시네이션이나 왜곡이 없는지 원본 리뷰와 비교 검증
- **입력**: 보고서 초안 + 원본 리뷰 전체
- **출력**: `{verified: true/false, issues: ["문제점 목록"], suggestions: ["수정 제안"]}`

</details>

**과제 2: 컨텍스트 윈도우 계산**

자신이 사용하는 AI 모델의 컨텍스트 윈도우를 확인하고:

1. 전체 컨텍스트 윈도우 크기: _______
2. 인스트럭션에 할당할 최대 토큰 (20-30%): _______
3. 현재 인스트럭션 크기 추정: _______
4. 분리 필요 여부 판단 및 근거: _______
...
- 텍스트를 붙여넣으면 토큰 수 확인 가능

**과제 3: 에이전트 설계 종합 연습**

자신의 업무에서 반복되는 복잡한 작업 하나를 선택하여 다음을 수행하세요:

1. **문제 파악**: 5.1절의 5가지 한계 중 현재 작업에서 겪는 문제 식별
2. **해결 방안**: 5.2절의 해결 방법론 적용
   - 할루시네이션 → 근거 기반 책임 + 검증 역할
   - 컨텍스트 윈도우 → 역할 분할
   - 비일관성 → 페르소나 + 구조화된 출력
   - 유지보수 → 모듈화
3. **분리 결정**: 5.3절의 3가지 관점(전문성, 재사용성, 응집도)으로 분석
4. **최종 설계**: 에이전트 목록과 각각의 역할, 제약, 페르소나 작성
