...
**해결 방향:**
- 작업을 작은 단계로 나누어 각 단계의 지시를 단순하게 유지
- 각 단계가 하나의 명확한 목표만 갖도록 설계
...
**해결 방향:**
- 생성과 검증을 분리하여 사실 확인 단계 추가
- 구체적인 출처 요구 및 참조 가능한 정보만 사용하도록 제한
...
**해결 방향:**
- 명확한 출력 형식과 템플릿 제공
- 구조화된 데이터 형식(JSON, YAML 등) 활용으로 일관성 확보
...
**해결 방향:**
- 관심사 분리(SoC) 원칙으로 역할별 인스트럭션 분할
- 모듈화된 인스트럭션 설계로 재사용성과 유지보수성 향상
...
우리는 AI 협업의 4가지 제약 조건을 배웠습니다:
1. 긴 작업에서 초반 지시 망각
2. 사실이 아닌 정보 생성 가능성 (할루시네이션)
3. 동일 질문에 대한 비일관적 응답
4. 복잡한 프롬프트의 유지보수 어려움

이러한 제약은 단일 프롬프트의 규모가 커질수록 더욱 두드러지지만, **인스트럭션 설계를 통해 관리할 수 있습니다.**
...
**핵심 메시지**: AI의 제약을 인정하고 인스트럭션으로 관리하는 것이 체계적 자동화의 시작입니다.
