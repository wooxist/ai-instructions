...
예를 들어, '명확성'과 '구체성'은 [산출물 중심(Output-Driven)](glossary.md#산출물-중심-output-driven) 시스템 설계 원칙의 구체적인 실행 방법이며, '단계성'은 복잡한 문제를 나누어 해결하는 **원자성(Atomicity)** 및 [관심사 분리(SoC)](glossary.md#soc-separation-of-concerns)와 깊이 관련됩니다. 즉, 3장의 기본 원칙들이 '전술'이라면, 4장의 시스템 설계 원칙들은 이 전술들을 이끄는 '전략'입니다.
...
4장에서는 인스트럭션 시스템 설계의 핵심 원칙들을 배웠습니다. 11가지 원칙을 구조적·실행·검증 세 카테고리로 나누어 살펴보았으며, 상황에 따라 어떤 원칙을 우선 적용해야 하는지에 대한 매트릭스와 체크리스트를 제공했습니다. 이제 다음 장에서는 이러한 원칙들을 실제 에이전트 설계에 어떻게 적용하는지 구체적으로 알아봅니다.
