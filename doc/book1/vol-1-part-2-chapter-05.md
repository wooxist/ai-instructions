...
3장에서 다룬 '효과적인 지시의 4가지 기본 원칙(명확성, 구체성, 단계성, 제약 설정)'은 개별 프롬프트와 인스트럭션 작성의 기초가 되는 실용적인 지침입니다. 이 장에서 다룰 **시스템 설계 원칙**은 한 단계 더 나아가, 이러한 좋은 인스트럭션들을 어떻게 '체계'로 확장하고 일관되게 유지할 것인지에 대한 전략적 청사진을 제공합니다.
...
다음으로 [SoC (Separation of Concerns)](glossary.md#soc-separation-of-concerns), '관심사 분리' 원칙입니다. 인스트럭션 설계에서 AI의 역할을 정의하는 부분, 처리 방법을 지시하는 부분, 결과물의 형식을 지정하는 부분을 분리하면, 각 부분을 독립적으로 수정하고 재사용하기가 훨씬 수월해집니다. 이 책의 5장이 '역할과 제약'을, 6장이 '입력과 출력'을 다루는 것도 이 원칙을 따른 것입니다.
...
- **[5장. 역할(Agent)과 제약(Constraint) 설계](vol-1-part-2-chapter-05.md):** 인스트럭션을 수행할 주체인 AI의 역할을 정의하고, 그 역할이 지켜야 할 규칙을 설정합니다.
- **[6장. 입력과 출력 설계](vol-1-part-2-chapter-06.md):** 에이전트가 사용할 '재료(입력)'와 만들어낼 '완성품(출력)'의 명확한 명세를 정의합니다.
- **[7장. 워크플로우 설계](vol-1-part-3-chapter-07.md):** 설계된 여러 에이전트들을 유기적으로 연결하여 복잡한 업무를 자동화하는 '협력 방식'을 정의합니다.
