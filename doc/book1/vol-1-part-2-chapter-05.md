...
### 이 장에서 배우는 것
- 좋은 설계를 위한 상위 원칙(SSOT, SoC 등)을 먼저 학습하여 전체적인 관점을 잡고, 좋은 인스트럭션을 일관되게 만들고 유지·개선하기 위한 시스템 설계 원칙을 익힙니다.
- 각 원칙의 좋은 예와 나쁜 예, 그리고 짧은 체크리스트를 살펴봅니다.
- 이 원칙들이 이후 장(역할, 입/출력, 처리 방법)에 어떻게 적용되는지 이해합니다.

## 제약을 다루는 체계적 접근

4장에서 우리는 AI의 현실적 제약과 실용적 해결책을 살펴봤습니다.
이제 이런 질문이 생깁니다:

"왜 이런 해결책들이 효과적일까?"
"새로운 문제를 만나면 어떻게 해결책을 찾을까?"

답은 **설계 원칙**에 있습니다.

4장의 해결책들은 사실 검증된 설계 원칙의 적용 사례입니다:

| 4장의 제약 | 4장의 해결 방법 | 이 장의 설계 원칙 |
|------------|----------------|----------------|
| Context Window 한계 | 작업 단계별 분할 | **원자성(Atomicity)** |
| | 핵심 규칙 구조화 | **MECE** |
| 할루시네이션 | 근거 기반 생성 | **SSOT** |
| | 생성-검증 패턴 | **피드백 루프** |
| 비일관성 | 출력 템플릿 제공 | **산출물 중심** |
| | 예시 기반 학습 | **컨텍스트 명시성** |
| 유지보수 어려움 | 역할별 분리 | **SoC** |
| | 버전 관리 | **투명성·추적성** |

이 장에서는 11가지 핵심 설계 원칙을 배워,
어떤 제약이든 체계적으로 해결할 수 있는 역량을 기릅니다.

3장에서 다룬 '효과적인 지시의 기본 원칙'은 개별 프롬프트와 인스트럭션 작성의 기초가 되는 실용적인 지침입니다. 이 장에서 다룰 **시스템 설계 원칙**은 한 단계 더 나아가, 이러한 좋은 인스트럭션들을 어떻게 '체계'로 확장하고 일관되게 유지할 것인지에 대한 전략적 청사진을 제공합니다.

예를 들어, '명확성'과 '구체성'은 [산출물 중심(Output-Driven)](glossary.md#산출물-중심-output-driven) 시스템 설계 원칙의 구체적인 실행 방법이며, '단계성'은 복잡한 문제를 나누어 해결하는 **원자성(Atomicity)** 및 [관심사 분리(SoC)](glossary.md#soc-separation-of-concerns)와 깊이 관련됩니다. 즉, 3장의 기본 원칙들이 '전술'이라면, 5장의 시스템 설계 원칙들은 이 전술들을 이끄는 '전략'입니다.

## 5.1 구조적 원칙: 설계의 뼈대 세우기
...
가장 먼저 고려해야 할 원칙은 [SSOT (Single Source of Truth)](glossary.md#ssot-single-source-of-truth)[^1], 즉 '단일 진실 공급원'입니다. 이는 인스트럭션 시스템의 일관성과 유지보수성을 보장하는 핵심 원칙으로, 주로 두 가지 방식으로 적용됩니다.

**첫째, 인스트럭션 설계 자체의 중복을 제거합니다.** 이는 마치 여러 문서에서 반복되는 회사 주소를 한 곳에만 적어두고, 주소가 바뀔 때 그 한 곳만 수정하면 모든 문서에 반영되게 하는 것과 같습니다. 여러 에이전트가 공통으로 사용하는 규칙, 템플릿, 출력 형식 정의 등을 별도의 공통 파일(예: `schemas/common.json`과 같은 구조화된 템플릿 파일)로 분리하고, 각 인스트럭션이 이를 참조하게 만드는 방식입니다. 이는 소프트웨어 개발의 **DRY(Don't Repeat Yourself)** 원칙과 같습니다.
...
다음으로 [SoC (Separation of Concerns)](glossary.md#soc-separation-of-concerns), '관심사 분리' 원칙입니다. 인스트럭션 설계에서 AI의 역할을 정의하는 부분, 처리 방법을 지시하는 부분, 결과물의 형식을 지정하는 부분을 분리하면, 각 부분을 독립적으로 수정하고 재사용하기가 훨씬 수월해집니다. 이 책의 7장이 '역할과 제약'을, 6장이 '입력과 출력'을 다루는 것도 이 원칙을 따른 것입니다.

또한, [MECE (Mutually Exclusive, Collectively Exhaustive)](glossary.md#mece-mutually-exclusive-collectively-exhaustive) 원칙을 적용하면 내용을 중복 없이, 빠짐없이 구성할 수 있습니다. 예를 들어, 오류 유형을 '입력 오류'와 '처리 오류'로 명확히 나누면, 각 유형에 대한 대응 전략을 체계적으로 수립할 수 있습니다.
...
## 5.2 실행 원칙: AI와 함께 일하는 방식 정의하기

튼튼한 뼈대 위에 살을 붙이는 과정, 즉 AI와 '어떻게 일할 것인가'에 대한 원칙도 중요합니다. 좋은 결과는 좋은 실행 과정에서 나옵니다.

가장 중요한 실행 원칙은 [산출물 중심(Output-Driven)](glossary.md#산출물-중심-output-driven)[^5] 접근법입니다. 이는 과정보다 최종 결과물의 요건을 먼저 명확히 정의하는 것입니다. "충분히 자세히"와 같은 모호한 지시 대신, "필수 필드 3개를 포함하는 JSON 형식으로 출력"처럼 기계가 검증할 수 있는 출력 사양을 제시하면, AI는 목표를 향해 더 정확하게 움직입니다. 여기에 [멱등성 (Idempotency)](glossary.md#멱등성-idempotency) 개념을 적용하면, 같은 입력에 대해 항상 같은 출력이 나오도록 보장할 수 있습니다. 이는 재현 가능성과 신뢰성을 높이는 핵심 요소입니다.
...
마지막으로 [Human-in-the-Loop](glossary.md#human-in-the-loop)(인간 참여)는 중요한 의사결정에 반드시 사람의 검토를 포함시키는 원칙입니다. 특히 배포, 승인, 법적 판단과 같은 중요한 단계에서는 AI의 제안을 참고하되 최종 결정은 사람이 내려야 합니다. 여기에 **실패 안전(Fail-safe)** 개념을 더하면, 오류가 발생했을 때 자동으로 이전의 안정된 상태로 롤백하거나, 안전한 기본값으로 복귀하도록 설계할 수 있습니다. 이는 AI와의 협업에서 발생할 수 있는 리스크를 관리하고, 최종적인 책임은 인간에게 있음을 명확히 하는 중요한 원칙입니다.

## 5.4 시스템 설계 원칙 요약표

지금까지 살펴본 시스템 설계 원칙들을 카테고리별로 정리하면 다음과 같습니다.

| 카테고리 | 원칙 | 핵심 내용 | 적용 예시 |
|---|---|---|---|
| **구조적 원칙** | [SSOT](glossary.md#ssot-single-source-of-truth) | 핵심 정의·템플릿을 단일 출처에서 관리하고 중복 제거 | 공통 출력 템플릿 파일(예: `schemas/common.json`)로 분리하여 참조 |
...
| **실행 원칙** | [산출물 중심](glossary.md#산출물-중심-output-driven) | 과정보다 최종 결과물 요건을 먼저 정의하고, 같은 입력에 같은 출력 보장 | "출력은 반드시 지정된 형식(JSON 구조 등)을 따라야 한다"고 명시 |
...
모든 상황에서 11개의 설계 원칙을 전부 적용할 필요는 없습니다. 작업의 특성에 따라 필수적으로 적용해야 할 원칙과 선택적으로 고려할 원칙이 다릅니다. 다음 매트릭스와 체크리스트를 활용하여 상황에 맞는 원칙을 선택하세요.
...
**범례:**
- **모두**: 해당 카테고리의 모든 원칙 적용 (예: 실행 원칙 '모두' = 산출물 중심, 피드백 루프, 점진적 개선, 컨텍스트 명시성 전체)
- **-**: 선택적 적용 (필수 아님)
- **명시된 원칙**: 해당 원칙 필수
...
지금까지 살펴본 시스템 설계 원칙들은 좋은 인스트럭션 시스템을 구축하기 위한 '지도'와 같습니다. 이제 이 지도를 바탕으로, 이어지는 장들에서는 실제 인스트럭션 시스템을 구축하는 구체적인 설계 방법론을 알아봅니다.

- **[6장. 입력과 출력 설계](vol-1-part-2-chapter-06.md):** 에이전트가 사용할 '재료(입력)'와 만들어낼 '완성품(출력)'의 명확한 명세를 정의합니다.
- **[7장. 단일 에이전트 설계](vol-1-part-2-chapter-07.md):** 인스트럭션을 수행할 주체인 AI의 역할을 정의하고, 그 역할이 지켜야 할 규칙을 설정합니다.
- **[8장. 워크플로우 설계](vol-1-part-2-chapter-08.md):** 설계된 여러 에이전트들을 유기적으로 연결하여 복잡한 업무를 자동화하는 '협력 방식'을 정의합니다.

---
## 실습 체크리스트

**개념 이해:**
- [ ] 11가지 시스템 설계 원칙을 3가지 카테고리(구조적, 실행, 검증·책임)로 분류할 수 있다
- [ ] SSOT, SoC, MECE, 원자성 등 구조적 원칙의 차이를 설명할 수 있다
- [ ] 산출물 중심, 피드백 루프, 점진적 개선 등 실행 원칙의 의미를 이해했다
- [ ] 상황별 필수 원칙 매트릭스의 구조를 이해했다

**실습 능력:**
- [ ] 자신의 작업에 적용해야 할 필수 설계 원칙을 선택할 수 있다
...
**과제 1: 시스템 설계 원칙 분류 및 적용** _(예상 소요 시간: 15분 / 난이도: 하)_

다음 시나리오에 적용해야 할 시스템 설계 원칙을 시스템 설계 원칙 요약표(4.4절)를 참고하여 선택하고, 그 이유를 설명하세요:
...
**과제 2: 원칙 적용 실습** _(예상 소요 시간: 30분 / 난이도: 중)_
...
**과제 3: 상황별 원칙 선택 연습** _(예상 소요 시간: 20분 / 난이도: 중)_
...
체크리스트 결과를 바탕으로 최종적으로 적용할 시스템 설계 원칙 목록을 작성하세요.

**과제 4: 실무 적용** _(예상 소요 시간: 40분 / 난이도: 상)_
...
5장에서는 인스트럭션 시스템 설계의 핵심 원칙들을 배웠습니다. 11가지 원칙을 구조적·실행·검증 세 카테고리로 나누어 살펴보았으며, 상황에 따라 어떤 원칙을 우선 적용해야 하는지에 대한 매트릭스와 체크리스트를 제공했습니다. 이제 다음 장에서는 이러한 원칙들을 실제 에이전트 설계에 어떻게 적용하는지 구체적으로 알아봅니다.
