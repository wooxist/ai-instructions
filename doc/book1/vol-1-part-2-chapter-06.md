...
5장에서 우리는 AI의 본질적 한계와 복잡한 작업을 작은 단계로 나누는 것의 중요성을 배웠습니다. 하지만 아무리 뛰어난 전문가라도, 작업에 필요한 '재료(입력)'가 잘못되거나 만들어야 할 '완성품(출력)'에 대한 정의가 모호하다면 제대로 된 결과물을 낼 수 없습니다. 입력과 출력 설계는 바로 이 '재료'와 '완성품'에 대한 명확한 약속, 즉 **명세(Specification)**를 만드는 과정입니다.
...
## 👁️ 다음 장 미리보기: 7장. 프로세스 설계와 워크플로우 패턴

### 개별 에이전트를 연결하여 복잡한 문제를 해결하세요

6장에서 우리는 명확한 **입출력 명세**를 설계하는 법을 배웠습니다. 하지만 대부분의 실무 문제는 단일 에이전트만으로는 해결하기 어렵습니다. 여러 전문가가 **협력**해야만 풀 수 있는 복잡한 작업들이 많습니다.

**7장에서는 다음을 배웁니다:**

- **파이프라인(Pipeline)**: 여러 에이전트가 순차적으로 작업을 처리하는 패턴
- **생성-검증(Generate-and-Verify)**: 품질 관리를 위한 피드백 루프 패턴
- **라우팅(Routing)**: 상황에 따라 다른 에이전트로 분기하는 패턴
- **workflow.yaml**: 코드 없이 설정 파일로 워크플로우 정의하기

**구체적인 예시:**
블로그 포스트를 자동으로 작성하는 시스템을 만든다고 가정해봅시다:

1. **리서치 에이전트**: 최신 동향 조사
2. **초안 작성 에이전트**: 리서치 결과로 초안 작성
3. **검수 에이전트**: 문법/사실 확인
4. **최종 편집 에이전트**: 검수 피드백 반영

각 에이전트는 6장에서 배운 **구조화된 입출력**을 통해 서로 소통합니다. 이것을 **‘산출물 인터페이스(Artifact Interface)’**라고 부르며, 7장에서는 이를 기반으로 복잡한 다중 에이전트 시스템을 구축하는 방법을 배웁니다.

**혼자서는 해결할 수 없던 문제들을, 협력으로 해결하는 법을 배울 준비가 되셨나요?**

**준비되셨다면, [7장: 프로세스 설계와 워크플로우 패턴](07-process-workflow.md)로 이동하세요!**
