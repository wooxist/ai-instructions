...
> **메타 에이전트 지시사항:**
> 새로운 목표를 받으면, 먼저 아래 정의에 따라 **조직 규모**를 결정하고, 목표의 **복잡도**를 판단하라. 그 다음, 매트릭스에 명시된 **필수 원칙**들을 이번 설계에 반드시 적용해야 한다.

- **단일 에이전트 (1개):** 하나의 워커 에이전트가 독립적으로 작업을 완료하는 경우.
- **단위 조직 (2-5개):** 하나의 아키텍트가 여러 워커들을 지휘하여 협업하는 경우.
- **복합 조직 (6개 이상):** 여러 아키텍트(팀)가 병렬적으로 협업하여 거대 목표를 수행하는 경우.
...
> **메타 에이전트 지시사항:**
> 모든 에이전트 인스트럭션은 아래 8가지 구성 요소를 **반드시 포함**하여 구현해야 한다. 이 구조는 2.1절의 메타 원칙들을 구체화하여, 모든 에이전트가 예측 가능하고 일관되게 작동하도록 보장하는 핵심 프레임워크이다.

```markdown
# [인스트럭션 제목]

## 1. 목적 (Purpose)
- 이 인스트럭션을 통해 달성하려는 최종 목표를 한 문장으로 정의합니다. (메타 원칙: 컨텍스트 명시성)

## 2. 역할 (Role)
- AI가 어떤 전문가의 역할을 수행해야 하는지 지정합니다. (메타 원칙: SoC, 컨텍스트 명시성)

## 3. 입력 (Input)
- AI가 처리해야 할 데이터의 형식, 구조, 출처를 설명합니다. (메타 원칙: 산출물 중심)

## 4. 처리 방법 (Process)
- 작업을 수행할 구체적인 절차나 단계를 순서대로 지시합니다. (메타 원칙: 원자성, 피드백 루프)

## 5. 출력 (Output)
- 결과물이 어떤 형식(JSON, Markdown 등)과 내용을 가져야 하는지 명시합니다. (메타 원칙: 산출물 중심)

## 6. 제약 조건 (Constraints)
- 반드시 지켜야 할 한계(분량, 시간, 비용)나 피해야 할 조건(금지 사항)을 설정합니다. (메타 원칙: 윤리적 경계, Human-in-the-Loop)

## 7. 평가 기준 (Evaluation)
- 결과물의 좋고 나쁨을 판단할 수 있는 구체적이고 검증 가능한 기준을 제공합니다. (메타 원칙: 피드백 루프, 산출물 중심)

## 8. 기초 지식 (Principles)
- 작업 전반에 걸쳐 AI가 항상 참고해야 할 기본 원칙이나 배경지식을 정의합니다. (메타 원칙: SSOT, 투명성·추적성)
```
