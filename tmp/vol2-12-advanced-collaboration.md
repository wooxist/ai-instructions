...
이 장은 **고급 협업 아키텍처**(★★★)를 다룹니다. 10장을 시작하기 전에, 당신의 준비도를 확인하세요.

### 필수 선수 학습: 7장 (★★☆)

**반드시 이해해야 하는 개념** — [7장. 프로세스 설계와 워크플로우 패턴](07-process-workflow.md)
- 파이프라인, 생성-검증, 라우팅 패턴
- `workflow.yaml`로 워크플로우 정의하기
- 1-2개 에이전트 협업 경험

✅ **7장만 학습한 경우**: 
- 가능하지만 난이도 격차가 큽니다 (★★☆ → ★★★)
- 디렉토리 구조와 상태 관리 개념이 새로움
- **권장**: 먼저 [7.5장](07a-intermediate-collaboration.md)을 학습하고 오세요

---

### 강력 권장: 7.5장 (★★★)

**10장을 이해하는 데 필수적인 개념** — [7.5장. 중간 규모 협업과 파일 기반 관리](07a-intermediate-collaboration.md)
- 디렉토리 기반 작업 관리 (`/tasks`, `/outputs`, `/shared`)
- `status.json`을 사용한 상태 추적
- JSON 기반 산출물 인터페이스
- 3-4개 에이전트 팀 협업

✅ **7.5장도 학습한 경우**: 
- 10장 학습이 훨씬 수월합니다 (점진적 난이도 상승)
- 디렉토리 구조 확장 개념을 이미 이해함
- 상태 관리 경험이 있어 복잡도 증가 대비 가능

---

### 10장에서 다룰 범위

10장은 **6개 이상의 에이전트**가 필요한 대규모 협업 시스템을 다룹니다:

**핵심 개념** (모두 새롭습니다):
- ✨ 메타/아키텍트/워커 **계층 구조**
- ✨ 에이전트가 다른 에이전트를 **동적으로 생성**
- ✨ `/agents`, `/workflows`, `/jobs` 분리된 **다층 디렉토리**
- ✨ `_job_log.json`으로 **다중 작업 관리**

**복잡도 비교**:
```
7장:   1-2개 에이전트, 평면 구조           (★★☆)
7.5장: 3-4개 에이전트, 1층 디렉토리          (★★★)
10장:  6개+ 에이전트, 3층 계층+동적 생성 (★★★)
```
...
**다음 모든 신호가 보이면 10장이 필요합니다**:
- ✅ 에이전트가 **5-6개 이상** 필요
- ✅ "누가 어떤 에이전트를 만들 것인가?"가 **동적으로 결정**됨
- ✅ 여러 프로젝트가 **동시에 진행**됨
- ✅ 에이전트 간 **계층 관계**가 필요함 (관리자 ↔ 실행자 ↔ 작업자)

**이 중 하나라도 해당되지 않는다르면?**

➡️ **돌아가세요**:
- **3-4개 에이전트 수준**: [7.5장](07a-intermediate-collaboration.md)으로
- **1-2개 에이전트 수준**: [8장. 성능 최적화](08-performance.md)로
- **실제 사례 학습**: [11장. 실무 사례](11a-single-agent.md)로

> **중요**: 대부분의 실무 프로젝트는 3-4개 에이전트(7.5장)로 충분합니다. 10장의 복잡한 구조는 **실제로 필요할 때만** 도입하세요.

---

### 📚 학습 준비도 체크리스트

10장을 시작하기 전에 다음을 확인하세요:

**필수 (7장)**:
- [ ] 파이프라인 패턴을 설명할 수 있다
- [ ] 생성-검증 패턴을 설명할 수 있다
- [ ] 라우팅 패턴의 필요성을 이해한다
- [ ] `workflow.yaml`로 워크플로우를 정의할 수 있다

**강력 권장 (7.5장)**:
- [ ] 디렉토리 구조 (`/tasks`, `/outputs`)를 설계할 수 있다
- [ ] `status.json`으로 상태를 추적할 수 있다
- [ ] JSON 기반 산출물 인터페이스를 이해한다
- [ ] 3-4개 에이전트 팀 협업을 경험했다

**준비 완료!** 10장을 시작하세요. 행운을 빌어요! 🚀
...
- **워크플로우 (Workflow):** 에이전트들이 작업을 수행하는 순서와 방식을 정의한 프로세스입니다.
- **[산출물 (Artifact)](glossary.md#산출물-artifact):** 워크플로우를 통해 생성되는 결과물입니다. (예: 코드, 문서, 보고서)
...
위의 동적 실행 프로세스를 통해 각 계층 에이전트의 역할이 명확해집니다.
...
sequenceDiagram
    ...
    Note over J: 산출물(JSON)은 다음 단계의 입력 인터페이스입니다
```
...
- [ ] 메타/아키텍트/워커의 역할과 책임을 명확히 구분할 수 있다
- [ ] 산출물 인터페이스(JSON/YAML)로 에이전트 간 계약을 정의할 수 있다
- [ ] 작업(Job) 디렉터리 구조 및 로그 설계를 통해 상태 추적이 가능하다
...
## 🔮 다음 장 미리보기: 11장 1부 - 단일 에이전트 설계

**"복잡한 시스템 아키텍처를 배웠다면, 이제 실전 사례로 적용해봅시다."**

10장에서는 **6개 이상의 에이전트**가 협력하는 고급 아키텍처를 배웠습니다. 하지만 "언제 이런 복잡한 구조를 쓸까?", "단일 에이전트로 충분한 경우는?"같은 현실적인 질문이 남습니다. 11장은 바로 이 질문에 답하는 **실전 사례집**입니다.

### 11장에서 배울 핵심 내용

**📑 실전 사례의 세 가지 차원**
- **에이전트 수**: 단일 (1개) → 단위 조직 (3-5개) → 복합 조직 (6개+)
- **복잡도**: 간단/일상 → 표준/전문 → 복잡/중요
- **9가지 실전 사례**: 이메일 요약 부터 신제품 캐페인까지

**👤 11.1부: 단일 에이전트 패턴**
- **사용자가 메타 에이전트 역할**: 사례 1에서 이해하기
- **단순하지만 강력한 패턴**: 대부분의 업무는 1개 에이전트로 해결 가능
- **3가지 복잡도**: 이메일 요약 (간단) → 마케팅 문구 (표준) → 회의록 분석 (복잡)

**📦 11.2부: 단위 조직 패턴**
- **3-5개 에이전트 팀**: 소셜 미디어 콘텐츠 발행
- **7.5장의 개념 적용**: 디렉토리 구조, 상태 추적
- **실용성 최적화**: 대부분의 팀 프로젝트는 여기서 해결

**🏭 11.3부: 복합 조직 패턴**
- **6개+ 에이전트**: 신제품 출시 캐페인 (종합 사례)
- **10장 전체 개념 통합**: 메타/아키텍트/워커 계층
- **엔터프라이즈 수준**: 언제 이 복잡도가 필요한가?
...
|---|---|
| 계층 구조 설명 | 9가지 사례로 적용 |
| 산출물 인터페이스 | JSON 스키마 예시 |
| 작업(Job) 관리 | 실제 디렉토리 구조 |
| 언제 써야 하는가? | 각 패턴의 사용 시기 |

**학습 권장 경로 ✅**: 
1. **11.1부**: 먼저 단일 에이전트 3가지 사례 학습
2. **11.2부**: 팀 협업이 필요한가? 단위 조직 학습
3. **11.3부**: 엔터프라이즈급 프로젝트라면 복합 조직 학습

**준비되셨나요?** [11장 1부: 단일 에이전트 설계](11a-single-agent.md)로 시작하세요! 🚀
